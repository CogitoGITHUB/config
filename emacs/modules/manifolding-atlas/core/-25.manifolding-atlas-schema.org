#+TITLE: manifolding-atlas-manifolding-atlas-schema
#+priority:  -25
* manifolding-atlas-manifolding-atlas-schema
:PROPERTIES:
:CATEGORY: manifolding-atlas-core
:END:
#+begin_src emacs-lisp
;;; manifolding-atlas-schema.el --- Note schema definition and validation -*- lexical-binding: t; -*-
;;
...
;; engine (`manifolding-atlas-schema-validate' and friends) is layered on top in the
;; same module.
;;
...
;; Example:
;;
;;   (manifolding-atlas-schema-define 'wine
...
;; `manifolding-atlas-schema-define' for the full merge rules.
;;
;;; Code:
...
(cl-defstruct (manifolding-atlas-schema (:constructor manifolding-atlas-schema--create)
                             (:copier nil))
...
(defvar manifolding-atlas-schema--registry (make-hash-table :test 'eq)
  "Registry of defined schemas, keyed by name symbol.")

(defun manifolding-atlas-schema--merge-fields (field-lists)
...
(cl-defun manifolding-atlas-schema-define (name &key predicate fields include)
...
symbol, a `manifolding-atlas-schema' object, or a list of either.  The included
...
`manifolding-atlas-schema' object."
  (unless (symbolp name)
  ...
  (let* ((includes (cond
                    ((null include) nil)
                    ((or (symbolp include) (manifolding-atlas-schema-p include)) (list include))
                    ((listp include) include)
                    (t (error "Schema :include must be a symbol, a schema, or a list of them: %S"
                              include))))
         (inherited (mapcar (lambda (s)
                              (manifolding-atlas-schema-fields (manifolding-atlas-schema--resolve s)))
                            includes))
         (include-names (mapcar (lambda (s)
                                  (if (manifolding-atlas-schema-p s) (manifolding-atlas-schema-name s) s))
                                includes))
         (schema (manifolding-atlas-schema--create
                  :name name
                  :predicate predicate
                  :includes include-names
                  :fields (manifolding-atlas-schema--merge-fields
                           (append inherited (list fields))))))
    (puthash name schema manifolding-atlas-schema--registry)
    schema))

(defun manifolding-atlas-schema-get (name)
  "Return the registered schema called NAME, or nil."
  (gethash name manifolding-atlas-schema--registry))

(defun manifolding-atlas-schema-list ()
  "Return the list of registered schema names."
  (let (names)
    (maphash (lambda (k _v) (push k names)) manifolding-atlas-schema--registry)
    (nreverse names)))

(defun manifolding-atlas-schema-unregister (name)
  "Remove the schema called NAME from the registry."
  (remhash name manifolding-atlas-schema--registry))

(defun manifolding-atlas-schema--resolve (schema-or-name)
  "Return a `manifolding-atlas-schema' from SCHEMA-OR-NAME.

SCHEMA-OR-NAME is either a `manifolding-atlas-schema' object or the symbol name of
a registered schema.  Signals an error when a name is not registered."
  (cond
   ((manifolding-atlas-schema-p schema-or-name) schema-or-name)
   ((symbolp schema-or-name)
    (or (manifolding-atlas-schema-get schema-or-name)
        (error "No schema registered as %S" schema-or-name)))
   (t (error "Not a schema or schema name: %S" schema-or-name))))

;;; Field access

(defun manifolding-atlas-schema-field-value (note field)
...
(defun manifolding-atlas-schema-applies-p (note schema-or-name)
  "Return non-nil when SCHEMA-OR-NAME's predicate matches NOTE.

SCHEMA-OR-NAME is a `manifolding-atlas-schema' object or a registered schema name."
  (let ((schema (manifolding-atlas-schema--resolve schema-or-name)))
    (funcall (manifolding-atlas-schema-predicate schema) note)))
...
Slots:
  ...
  type       - violation type symbol (see `manifolding-atlas-schema-validate')
  message    - human-readable description
  value      - the offending value, when applicable"
  note-id note-title schema field type message value)

(defun manifolding-atlas-schema--call-or-value (spec note)
...
(defun manifolding-atlas-schema--coerce (raw type)
...
(defun manifolding-atlas-schema--link-id (raw)
...
(defun manifolding-atlas-schema--violation (note schema field type message value)
...
MESSAGE describes the problem and VALUE is the offending value."
  (make-manifolding-atlas-violation
   ...
   :schema (manifolding-atlas-schema-name schema)
   :field (plist-get field :key)
   :type type
   :message message
   :value value))

(defun manifolding-atlas-schema--target-tags-violation (target id field note schema)
...
custom :validate function.  Returns the first problem found."
  (let* ((key (plist-get field :key))
         ...
         (one-of (manifolding-atlas-schema--call-or-value (plist-get field :one-of) note))
    ...
    (or
     ...
     (pcase type
       ...
       ((or 'note 'link)
        (let ((id (manifolding-atlas-schema--link-id raw)))
          (cond
           ((null id)
            (manifolding-atlas-schema--violation
             note schema field 'wrong-type
...
(defun manifolding-atlas-schema--field-violations (note field schema)
  "Return the list of violations for FIELD on NOTE against SCHEMA."
  (let* ((key (plist-get field :key))
         (required (manifolding-atlas-schema--call-or-value (plist-get field :required) note))
         (raws (manifolding-atlas-note-meta-get-list note key 'string)))
    (cond
     ((and required (null raws))
      (list (manifolding-atlas-schema--violation
             note schema field 'missing-required
             (format "Required field %S is missing" key) nil)))
     ((null raws) nil)
     (t (delq nil
              (mapcar (lambda (raw)
                        (manifolding-atlas-schema--value-violation raw field note schema))
                      raws))))))

(defun manifolding-atlas-schema-validate (note schema-or-name)
...
here (see `manifolding-atlas-schema-validate-all')."
  (let ((schema (manifolding-atlas-schema--resolve schema-or-name)))
    (cl-loop for field in (manifolding-atlas-schema-fields schema)
             append (manifolding-atlas-schema--field-violations note field schema))))

(defun manifolding-atlas-schema-validate-notes (notes schema-or-name)
...
schema applies to, filter with `manifolding-atlas-schema-applies-p' first, or use
`manifolding-atlas-schema-validate-all'."
  (let ((schema (manifolding-atlas-schema--resolve schema-or-name)))
    (cl-loop for note in notes
             append (manifolding-atlas-schema-validate note schema))))

(defun manifolding-atlas-schema-validate-all (schema-or-name)
...
is sugar for `manifolding-atlas-schema-validate-notes' over the matching notes."
  (let ((schema (manifolding-atlas-schema--resolve schema-or-name)))
    (manifolding-atlas-schema-validate-notes
     (manifolding-atlas-db-query (lambda (note) (manifolding-atlas-schema-applies-p note schema)))
     schema)))

;;; Authoring

(defun manifolding-atlas-schema-applicable (note)
  "Return the names of registered schemas whose predicate matches NOTE.
Schemas are tested in registration order."
  (cl-remove-if-not (lambda (name) (manifolding-atlas-schema-applies-p note name))
                    (manifolding-atlas-schema-list)))

(defun manifolding-atlas-schema-missing-fields (note schema-or-name &optional required-only)
...
:key, so this never proposes a field the note already carries."
  (let ((schema (manifolding-atlas-schema--resolve schema-or-name))
        (required nil)
        (optional nil))
    (dolist (field (manifolding-atlas-schema-fields schema))
      (when (null (manifolding-atlas-note-meta-get-list note (plist-get field :key) 'string))
        (if (manifolding-atlas-schema--call-or-value (plist-get field :required) note)
            (push field required)
          (unless required-only (push field optional)))))
    (append (nreverse required) (nreverse optional))))

(defun manifolding-atlas-schema-note-violations (note)
  "Return all violations of NOTE across the schemas that apply to it.

Validates NOTE against each schema named by `manifolding-atlas-schema-applicable'
...
`manifolding-atlas-schema-validate-all' and the shared entry point for surfacing a
...
widgets)."
  (cl-loop for name in (manifolding-atlas-schema-applicable note)
           append (manifolding-atlas-schema-validate note name)))

;;; Collection health

(cl-defstruct (manifolding-atlas-schema-health (:constructor manifolding-atlas-schema-health--create)
                                    (:copier nil))
...
(cl-defun manifolding-atlas-schema-collection-health (&optional (notes (manifolding-atlas-db-query)))
...
`manifolding-atlas-schema-health' structures, one per registered schema in
...
default `manifolding-atlas-db-query'; pass NOTES to aggregate an explicit set."
  (let ((by-name (make-hash-table :test 'eq))
    ...
    (dolist (name (manifolding-atlas-schema-list))
      (let ((includes (manifolding-atlas-schema-includes (manifolding-atlas-schema-get name))))
        (puthash name (manifolding-atlas-schema-health--create
    ...
    (dolist (note notes)
      (dolist (name (manifolding-atlas-schema-applicable note))
        (let ((h (gethash name by-name)))
          (when h
            (cl-incf (manifolding-atlas-schema-health-covered h))
            (let ((vs (manifolding-atlas-schema-validate note name)))
              (when vs
                (cl-incf (manifolding-atlas-schema-health-invalid h))
                (push (cons note vs) (manifolding-atlas-schema-health-invalid-notes h))))))))
    (dolist (name (manifolding-atlas-schema-list))
      (let ((h (gethash name by-name)))
        (setf (manifolding-atlas-schema-health-invalid-notes h)
              (nreverse (manifolding-atlas-schema-health-invalid-notes h)))
        (setf (manifolding-atlas-schema-health-included-by h)
              (nreverse (gethash name included-by)))
        (push h result)))
    (nreverse result)))

(provide 'manifolding-atlas-schema)
#+end_src
