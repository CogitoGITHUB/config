...
#+priority: 15
:PROPERTIES:
:header-args:emacs-lisp: :tangle manifolding-atlas-capture.el
:END:

A literate plugin for manifolding-atlas v2 providing two workflows:
1. Select text via avy → create a new manifolding-atlas note
...
:PROPERTIES:
:DESCRIPTION: Select any visible text with an avy jump and send it to a new manifolding-atlas note.
:END:
...
;; In your init.el, after manifolding-atlas and avy are loaded:
(load "path/to/manifolding-atlas-capture.el")
#+end_src
