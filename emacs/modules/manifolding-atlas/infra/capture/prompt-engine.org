    ...
    (dolist (spec my/manifolding-atlas-prompt-registry)
      (when (seq-intersection contexts (plist-get spec :contexts) #'eq)
        (let* ((key (plist-get spec :key))
               ...
               (defer (my/manifolding-atlas-collect--defer-p dspec))
...
#+end_src
*** SELECTION chooser
Opens the same transient toggle menu as the properties update command
\(SPC P), scoped to CONTEXTS. Returns registry KEYS to make interactive.
#+begin_src emacs-lisp
(defun my/manifolding-atlas-collect--choose-custom (contexts)
  "Ask which properties to customize for this capture.
The menu's body-template entry pre-runs the template chooser now,
so it is not asked again at finalize."
...
(defun my/manifolding-atlas-collect--selection-fast-level (specialized-contexts general-contexts)
  "Ask which keys fire; store them and return effective level 1."
