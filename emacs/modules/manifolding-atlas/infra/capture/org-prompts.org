...
(defun my/manifolding-atlas-org-prompt--session-finish ()
  "Drained the queue: drop the session and run its finalizer."
  (let ((finalize (plist-get my/manifolding-atlas-org-prompt--session
                             :finalize)))
  ...
  (let* ((note (plist-get my/manifolding-atlas-org-prompt--session :note))
    ...
    (if (not (buffer-live-p note-buf))
      ...
      (let ((win (split-window-below
                  (- (max 8 (min 16 (floor (* 0.30 (frame-height)))))))))
