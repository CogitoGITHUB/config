...
QUICK capture mode (fast level 3) skips every queued prompt."
...
QUICK capture mode (fast level 3) skips every parked prompt."
...
#+end_src
** Session open
Arms the picker: mode on, point at WARNING/default, highlight applied.
...
#+end_src
** Picker keymap
Q/C-g abort everything, q skips to WARNING, m toggles live apply.
#+begin_src emacs-lisp
...
#+end_src
** Picker minor mode
The buffer stays editable; only the keys change.
#+begin_src emacs-lisp
...
:kill-buffer behavior.  Content is safe on disk."
  ...
  (kill-buffer))
#+end_src
** Emulation-level keymap
Picker keys must win over modaled states, so the map rides in
emulation-mode-map-alists keyed on a buffer-local switch.
#+begin_src emacs-lisp
...
Same tree-slide/widen/mode dance as the session picker, minus the
