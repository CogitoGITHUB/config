...
These prompts fire when marking a heading as a rule via `my/manifolding-atlas-rules-mark-as-rule` (`SPC v k`). They use context `'(rule)`.
...
- Prompts fire on existing headings (via `my/manifolding-atlas-collect-prompts '(rule)`)
...
2. Call `(my/manifolding-atlas-register-prompt ...)` with `:contexts '(rule)`
