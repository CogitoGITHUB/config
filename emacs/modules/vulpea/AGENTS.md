...
- **Database:** `~/test/admin/vulpea.db` (root-owned `root:users 664`, excluded from git via `.gitignore`)
...
Live refs also get a `#+transclude: [[id:REFID][VALUE]]` directive inserted right under their owning note's heading (via `my/vulpea--add-live-transcludes`, advised on `vulpea-create`), so ref-note content shows inline through org-transclusion. `my/vulpea--ref-link-value` matches `[[id:ID][LABEL]]` only; plain values are skipped.
