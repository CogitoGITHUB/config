...
These prompts only fire for dictionary word notes (context `'(dictionary)`). They are collected via `(my/vulpea-collect-prompts-with-fast '(dictionary) '(file))` in `my/vulpea-dictionary-new-word`.
...
Dictionary prompts always fire at levels 0 and 1. They are the "specialized" contexts in `my/vulpea-collect-prompts-with-fast`.
...
2. Call `(my/vulpea-register-prompt ...)` with `:contexts '(dictionary)`
