[![ci](https://github.com/org-roam/manifolding-mind-map/actions/workflows/ci.yml/badge.svg)](https://github.com/org-roam/manifolding-mind-map/actions/workflows/ci.yml) [![MELPA](https://melpa.org/packages/manifolding-mind-map-badge.svg)](https://melpa.org/#/manifolding-mind-map)
...
# manifolding-mind-map: a graphical frontend for your org-roam Zettelkasten
...
- [Supporting manifolding-mind-map](#supporting-manifolding-mind-map)
  - [Feedback](#feedback)
...
For major new features/bugfixes we will update [changelog](https://github.com/org-roam/manifolding-mind-map/discussions/30).

## Installation

`manifolding-mind-map` is on MELPA!

manifolding-mind-map requires `org-roam`, `websocket`, `simple-httpd`, `f` and Emacs >= 27 for fast JSON parsing.

### package.el

```
M-x package-install manifolding-mind-map
```

No configuration is necessary when you use `package.el` to install MMM.
...
(use-package! manifolding-mind-map
    :after org-roam ;; or :after org
;;         normally we'd recommend hooking orui after org-roam, but since org-roam does not have
;;         a hookable mode anymore, you're advised to pick something yourself
;;         if you don't care about startup time, use
;;  :hook (after-init . manifolding-mind-map-mode)
    :config
    (setq manifolding-mind-map-sync-theme t
          manifolding-mind-map-follow t
          manifolding-mind-map-update-on-save t
          manifolding-mind-map-open-on-start t))

```

We recommend only loading manifolding-mind-map after loading org(-roam) as starting the server and making database requests can impact startup times quite a lot.
...
(use-package manifolding-mind-map
  :straight
    (:host github :repo "org-roam/manifolding-mind-map" :branch "main" :files ("*.el" "out"))
    :after org-roam
;;         normally we'd recommend hooking orui after org-roam, but since org-roam does not have
;;         a hookable mode anymore, you're advised to pick something yourself
;;         if you don't care about startup time, use
...
```emacs-lisp
(manifolding-mind-map-add-to-local-graph &optional id)
```
...
```emacs-lisp
(manifolding-mind-map-remove-from-local-graph &optional id)
```
...
MMM follows you around Emacs by default. To disable this, set

```emacs-lisp
(setq manifolding-mind-map-follow nil)
```

or disable the minor mode `manifolding-mind-map-follow-mode`.
...
```emacs-lisp
(setq manifolding-mind-map-update-on-save nil)
```
...
```emacs-lisp
(setq manifolding-mind-map-sync-theme nil)
```

Then call `M-x mmm-sync-theme`.

You can also provide your own theme if you do not like syncing nor like the default one. To do so, set `manifolding-mind-map-custom-theme` to an alist of (rather specific) variables, like so

```emacs-lisp
(setq manifolding-mind-map-custom-theme
    '((bg . "#1E2029")
        (bg-alt . "#282a36")
        (fg . "#f8f8f2")
        (fg-alt . "#6272a4")
...
You can optionally provide `(base1 . "#XXXXXX")` arguments after the last one to also set the background shades, otherwise MMM will guess based on the provides bg and fg.

### Open on start

By default, manifolding-mind-map will try to open itself in your default browser. To disable this, set

```emacs-lisp
(setq manifolding-mind-map-open-on-start nil)
```

## Disclaimers ‼

- We only support [org-roam v2](https://blog.jethro.dev/posts/org_roam_v2/); v1 will never be supported.
- As the name suggests, Org-Roam-UI only works with org-roam! If you organize your notes in some other form manifolding-mind-map cannot work, as it uses org-roam to fetch all the connections.
...
If your feature is not already on the [project board](https://github.com/org-roam/manifolding-mind-map/projects/2), please post minor feature requests such as "I want to be able to color this specific node" in [the minor feature requests discussion](https://github.com/org-roam/manifolding-mind-map/discussions/6) and _major_ feature requests (e.g. "I want to publish my graph) in [the major feature requests discussion](https://github.com/org-roam/manifolding-mind-map/discussions/66) or upvote those already posted, this way we can adjust our priorities somewhat!
...
When you open a note in Emacs, manifolding-mind-map will move to the corresponding node on the graph.

https://user-images.githubusercontent.com/21983833/127747187-7823a825-a2f8-449c-a0ec-1c5c525621dc.mp4

#### Theme syncing

Your gruvbox is only a `M-x mmm-sync-theme` away (or you can just select them in the settings).
...
# Supporting manifolding-mind-map

## Chat

We have a small telegram group for discussing manifolding-mind-map, you can join if you're cool.
https://t.me/+sEuL2qi37yw1NzQy

## Feedback

For feature suggestions, please make an issue or check out the discussions for [major](https://github.com/org-roam/manifolding-mind-map/discussions/66) and [minor features](https://github.com/org-roam/manifolding-mind-map/discussions/6).
For other feedback, please go to the [feedback discussion](https://github.com/org-roam/manifolding-mind-map/discussions/99), or open up a new one!

## Contribute 💪

The best way to support the continued development of manifolding-mind-map is to get involved yourself!
To get started, simply

```bash
git clone https://github.com/org-roam/manifolding-mind-map
yarn
yarn dev
```
...
If you are interested in being more closely involved with the project, go [here](https://github.com/org-roam/manifolding-mind-map/discussions/4) to have an onboarding call with a member of the core team.

We would ❤️ to have you on board!

## Donate

If you really really like manifolding-mind-map, you can make a [one-time donation or sponsor one of us monthly!](https://github.com/sponsors/tefkah/)
