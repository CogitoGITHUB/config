...
export const getNodeColor = ({
  ...
  const nc = (colorKey: string, subKey: string, op: number): string => {
    const c = highlightColors[colorKey]?.[subKey]
    return c ? c(op) : getThemeColor(colorKey || 'blue', theme)
  }
  ...
  if (tagColors && node?.tags?.some((tag) => tagColors[tag])) {
    ...
    return needsHighlighting
      ? nc(tagColor, tagColor, visuals.highlightFade * opacity)
      : nc(tagColor, visuals.backgroundColor, visuals.highlightFade * opacity)
  }
  if (visuals.citeNodeColor && node?.properties?.ROAM_REFS && node?.properties?.FILELESS) {
    return needsHighlighting
      ? getThemeColor(visuals.citeNodeColor, theme)
      : nc(visuals.citeNodeColor, visuals.backgroundColor, visuals.highlightFade * opacity)
  }
  if (visuals.refNodeColor && node.properties.ROAM_REFS) {
    return needsHighlighting
      ? getThemeColor(visuals.refNodeColor, theme)
      : nc(visuals.refNodeColor, visuals.backgroundColor, visuals.highlightFade * opacity)
  }
  const colorId = getNodeColorById({ id: node.id as string, cluster, coloring, linksByNodeId, visuals })
  if (!needsHighlighting) {
    return nc(colorId, visuals.backgroundColor, visuals.highlightFade * opacity)
  }
  if (!visuals.nodeHighlight) {
    return getThemeColor(colorId, theme)
  }
  return nc(colorId, visuals.nodeHighlight, opacity)
}
