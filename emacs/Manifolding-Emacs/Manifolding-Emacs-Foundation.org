...
** Loader block evaluator
Shallow on purpose: one small defun, easy to verify by eye.
EOF ends the block quietly; read/eval errors propagate LOUDLY.
#+begin_src emacs-lisp
(defun my/eval-loader-block (value n)
  "Eval every form in VALUE (Org src block N).
End-of-file ends the block quietly; other errors fail the boot."
  (with-temp-buffer
    (insert value)
    (goto-char (point-min))
    (while t
...
** Literate loader
Evaluates Manifolding-Emacs/manifolding-emacs.org — the package
manager — block by block, lexically, in document order.
...
(defun my/load-literate-loader (&optional file)
  ...
  (or file (setq file
                 (expand-file-name "manifolding-emacs.org"
                                   (locate-user-emacs-directory
                                    "Manifolding-Emacs/"))))
  (unless (file-exists-p file)
    (error "[init] literate loader missing: %s" file))
  (my/init-note "[init] literate loader…")
    ...
    (org-element-map tree '(src-block headline)
      (lambda (el)
        (pcase (org-element-type el)
          ...
          ('src-block
           (when (string=
                  (org-element-property :language el) "emacs-lisp")
             (setq n (1+ n))
             (my/eval-loader-block
              (org-element-property :value el) n))))))
