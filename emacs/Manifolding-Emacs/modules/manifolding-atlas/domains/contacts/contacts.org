...
(defvar my/manifolding-atlas-contacts-directory
  (expand-file-name "admin/contacts/" (my/manifolding-atlas-root-dir))
  "Directory (inside the vault, under admin/) where contact notes live, one per file.")

(defvar my/manifolding-atlas-contacts-tag "contact"
  "Tag marking a note as a contact.  Filters find and completion.")
#+end_src
...
#+begin_src emacs-lisp
(defun my/manifolding-atlas-contacts--files ()
  "Return all contact Org files under the contacts directory."
  (make-directory my/manifolding-atlas-contacts-directory t)
  (directory-files-recursively my/manifolding-atlas-contacts-directory "\\.org\\'"))

(defun org-contacts-files ()
  "Return Org files to use for contact management.
Overridden for Manifolding Atlas: scans contacts/ in the vault."
  (or (my/manifolding-atlas-contacts--files)
      (org-agenda-files t 'ifmode)))

(setq org-contacts-directory my/manifolding-atlas-contacts-directory)
(setq org-contacts-files (my/manifolding-atlas-contacts--files))
(org-contacts-org-complete-setup)
#+end_src
* Contact Commands
#+begin_src emacs-lisp
(defun my/manifolding-atlas-contacts--slugify (name)
  "Turn NAME into a safe file-name stem."
  (let ((s (downcase name)))
...
** Body capture opener
#+begin_src emacs-lisp
(defun my/manifolding-atlas-contacts--open-body-capture (note body)
...
** New contact
#+begin_src emacs-lisp
(defun my/manifolding-atlas-contacts-new (name)
  "Create a new contact note for NAME.
Prompts for fast mode level and general (file) prompts; tags the
note `contact' and stores it as admin/contacts/SLUG.org."
  (interactive "sContact name: ")
  (let* ((root my/manifolding-atlas-contacts-directory)
         (_ (make-directory root t))
         (slug (my/manifolding-atlas-contacts--slugify name))
         (file-name (concat "contacts/" slug))
         (prompts (my/manifolding-atlas-collect-prompts-with-fast '(contact) '()))
         (tags (append (list my/manifolding-atlas-contacts-tag slug)
         ...
         (note (my/manifolding-atlas--create-file-wrapper
                name file-name tags properties body)))
    (dolist (fn post-apply)
      (funcall fn note))
    (my/manifolding-atlas-contacts--open-body-capture note body)
    (setq org-contacts-files (my/manifolding-atlas-contacts--files))
    note))
#+end_src
** Find and default filter
#+begin_src emacs-lisp
(defun my/manifolding-atlas-contacts-find ()
  "Find a contact note via manifolding-atlas."
  (interactive)
  (manifolding-atlas-find
   :filter-fn
   (lambda (note)
     (member my/manifolding-atlas-contacts-tag
             (manifolding-atlas-note-tags note)))))

(defun my/manifolding-atlas--default-filter-exclude-contacts (note)
  "Return non-nil unless NOTE is a contact or internal ref note."
  (let ((path (manifolding-atlas-note-path note)))
    (and (not (member my/manifolding-atlas-contacts-tag
                      (manifolding-atlas-note-tags note)))
         (not (string-match-p "/admin/refs/" path)))))

(setq manifolding-atlas-find-default-filter
      #'my/manifolding-atlas--default-filter-exclude-contacts)
#+end_src
* Transient Menu
#+begin_src emacs-lisp
(require 'transient)

(transient-define-prefix my/manifolding-atlas-contacts-menu ()
  "Contacts actions."
  [["Contacts"
    ("n" "New contact"  my/manifolding-atlas-contacts-new)
    ("f" "Find contact" my/manifolding-atlas-contacts-find)]])
#+end_src
