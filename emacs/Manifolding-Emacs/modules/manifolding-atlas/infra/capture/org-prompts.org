...
** Avy to option
Manual one-shot jump: pick an option with avy and the cursor lands on
it (no auto-mark). The auto-armed marking loop (below) is the default
in sessions; this is the standalone / fallback jump.
#+begin_src emacs-lisp
...
#+end_src
** Avy marking loop
The session picker opens with avy ARMED BY DEFAULT. Letters are ONLY
jump targets — press an option's letter and it jumps there and marks
it live immediately. Avy stays alive: after each jump you can press
another option's letter to jump and mark that too (multi-select or
change selection). Only level-2 option headings can be marked.
...
Avy stays armed the whole time the picker is open: typing letters
jumps to an option and applies it live; you can keep jumping and re-
mark to change the selection. RET advances only when something is
marked; C-q quits, C-l makes live, C-e edits the property file."
  (require 'avy)
  (let ((exit-loop nil))
...
(defun my/manifolding-atlas--display-buffer-responsive (buf)
  "Show BUF in a side window on wide frames, bottom window on narrow
ones. Never silently swallows a split failure — always warns."
  (condition-case err
      (if (> (frame-width) 140)
          (let ((win (condition-case err2
                         (split-window-right (floor (* 0.65 (frame-width))))
                       (error (warn "atlas-display side: %s" (error-message-string err2)) nil))))
            (if win
                (progn (set-window-buffer win buf) (select-window win))
              (display-buffer buf '((display-buffer-reuse-window display-buffer-at-bottom) (window-height . 0.35)))))
        (let ((win (condition-case err2
                       (split-window-below (- (max 10 (floor (* 0.60 (frame-height))))))
                     (error (warn "atlas-display bottom: %s" (error-message-string err2)) nil))))
          (if win
              (progn (set-window-buffer win buf) (select-window win))
            (display-buffer buf '((display-buffer-reuse-window display-buffer-at-bottom) (window-height . 0.35))))))
    (error (warn "atlas-display outer: %s" (error-message-string err)) (ignore-errors (pop-to-buffer buf)))))
#+end_src
** Session display
...
(defun my/manifolding-atlas-org-prompt--session-open (req)
  ...
  (let* ((spec (plist-get req :spec))
    ...
    ;; Arm the avy-first marking loop: letters jump and mark by
    ;; default, only RET advances, C-q/C-l/C-e are the controls.
         ...
         (if (> (frame-width) 140)
             (condition-case err
                 (if (fboundp 'display-buffer-in-side-window)
                     (display-buffer-in-side-window buf '((side . right) (slot . 0) (window-width . 0.35)))
                   (let ((win (split-window-right (floor (* 0.65 (frame-width))))))
               ...
               (error (warn "atlas-ask-buffer right: %s" (error-message-string err)) (display-buffer buf '((display-buffer-reuse-window display-buffer-at-bottom) (window-height . 0.35)))))
           (condition-case err
               (let ((win (split-window-below (- (max 10 (floor (* 0.60 (frame-height))))))))
                 (if win (progn (set-window-buffer win buf) (select-window win)) (display-buffer buf '((display-buffer-reuse-window display-buffer-at-bottom) (window-height . 0.35)))))
             (error (warn "atlas-ask-buffer below: %s" (error-message-string err)) (display-buffer buf '((display-buffer-reuse-window display-buffer-at-bottom) (window-height . 0.35))))))
       (error (warn "atlas-ask-buffer outer: %s" (error-message-string err)) (pop-to-buffer buf)))
     (with-current-buffer buf
       (when (fboundp 'modaled-set-state)
         (modaled-set-state "atlas-pick")))
     (ignore-errors (recenter-top-bottom))
