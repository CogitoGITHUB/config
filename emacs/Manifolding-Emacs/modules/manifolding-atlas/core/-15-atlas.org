#+TITLE: manifolding-atlas-manifolding-atlas
#+priority:  -15
* manifolding-atlas-manifolding-atlas
:PROPERTIES:
...
(require 'manifolding-atlas-db-sync)
(require 'manifolding-atlas-db-schema-validation)
(require 'manifolding-atlas-mentions)
(require 'manifolding-atlas-meta)
(require 'manifolding-atlas-note)
(require 'manifolding-atlas-schema)
(require 'manifolding-atlas-select)
(require 'manifolding-atlas-tags)
   ...
   :meta (when schema
           (delq nil
                 (mapcar
                  ...
                  (manifolding-atlas-schema-fields (manifolding-atlas-schema--resolve schema)))))))
...
placeholder, so the author is still reminded of it."
  (let (values)
    (dolist (field fields)
      (let* ((key (plist-get field :key))
             (required (manifolding-atlas-schema--call-or-value
...
;;;###autoload
(defun manifolding-atlas-schema-insert-fields (&optional schema-or-name skeleton)
...
when point is inside one, otherwise the file-level metadata."
  ...
  (let* ((schema (or schema-or-name
         ...
         (fields (manifolding-atlas-schema-missing-fields note schema)))
    (if skeleton
...
;;;###autoload
(defun manifolding-atlas-schema-fix-violation (violation &optional bound)
...
value the way `manifolding-atlas-schema-insert-fields' does - offering :one-of
...
`manifolding-atlas-schema-validate' violation."
  (let* ((schema (manifolding-atlas-schema--resolve (manifolding-atlas-violation-schema violation)))
