#+TITLE: manifolding-atlas-manifolding-atlas-db-worker
#+priority:  -23
* manifolding-atlas-manifolding-atlas-db-worker
:PROPERTIES:
...
the `warning' action must run in the main process to be seen."
  (let ((filters manifolding-atlas-db-note-index-filter-functions))
    (or (null filters)
        (and (equal filters '(manifolding-atlas-db-schema-validation--filter))
             (or (eq (bound-and-true-p manifolding-atlas-db-schema-validation-action)
                     'silent)
                 (not (and (fboundp 'manifolding-atlas-schema-list)
                           (manifolding-atlas-schema-list))))))))
...
(provide 'manifolding-atlas-db-worker)
#+end_src
