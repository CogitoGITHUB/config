#+title: Manifolding Atlas Transclusion Plugin
#+priority: 15
...
Vendored engine: the org-transclusion package and its text-clone
dependency are embedded below as loader-managed source (no straight).
The RET / mouse-2 keymap lives on the transclusion overlay's `keymap'
property so it wins over org-mindmap-mode's minor-mode map.
#+begin_src emacs-lisp
(setq org-transclusion-extensions nil)
(defvar org-transclusion-indent-mode nil)
#+end_src
* Vendored text-clone engine
#+begin_src emacs-lisp
...
* Vendored org-transclusion engine
#+begin_src emacs-lisp
...
current buffer."
  (and-let* ((id (org-id-uuid))
    ...
    (let ((ov-tc (text-clone-make-overlay beg end)))
      ...
      (overlay-put ov-tc 'keymap org-transclusion-map))
...
#+begin_src emacs-lisp
(defun my/manifolding-atlas-transclusion-add (&optional only-body)
  "Select a manifolding-atlas note and insert a live transclusion at point.
...
the current heading:
  ...
  (let ((org-transclusion-exclude-elements (list 'property-drawer))
    ...
    (setq note   (manifolding-atlas-select "Transclude note: " :require-match t))
...
;; RET on a transclusion opens its source.  Ref notes (admin/refs/*) open
;; in an org-capture edit buffer for fast metadata edits; anything else
;; falls back to org-transclusion's own source opener.
;; org-transclusion.el reads `org-transclusion-indent-mode' as a
;; buffer-local value, but that var is only defined by the unloaded
;; org-transclusion-indent-mode extension — declare it so capture
...
#+begin_src emacs-lisp
(defun my/manifolding-atlas-transclusion-open ()
  "Open the source of the transclusion at point.
Ref notes open in org-capture (fast editing); otherwise fall back to
`org-transclusion-open-source'.  The `org-transclusion-id' text property
is an internal pairing UUID, so the real source id is parsed from
`org-transclusion-orig-keyword'."
  (interactive)
  (let* ((data (get-text-property (point) 'org-transclusion-orig-keyword))
         (link (and (plist-member data :link) (plist-get data :link)))
         (id (and (stringp link)
                  (string-match "\\[\\[id:\\([^] \t\n]+\\)\\]\\[" link)
                  (match-string-no-properties 1 link)))
         (file (and id (org-id-find-id-file id)))
         (ref-root (and (fboundp 'my/manifolding-atlas-root-dir)
                        (expand-file-name "admin/refs/" (my/manifolding-atlas-root-dir)))))
    (if (and id file ref-root
             (fboundp 'my/manifolding-atlas--edit-file-in-capture)
             (file-in-directory-p (expand-file-name file) ref-root))
        (my/manifolding-atlas--edit-file-in-capture file)
      (org-transclusion-open-source))))

(with-eval-after-load 'org-transclusion
  (define-key org-transclusion-map (kbd "RET")
    #'my/manifolding-atlas-transclusion-open)
  (define-key org-transclusion-map (kbd "<mouse-2>")
    #'my/manifolding-atlas-transclusion-open))

;; In-drawer transclusion support.  A `:property' transclude encodes a live
;; property as a `#+transclude:' line INSIDE a :PROPERTIES: drawer.  Rendering
;; a source headline (`* X') inside a drawer breaks org's drawer parsing, so
;; this keyword strips the leading asterisk, turning the transcluded content
;; into org-safe plain text inside the drawer.

#+end_src

#+begin_src emacs-lisp
(defun my/manifolding-atlas-transclusion-keyword-value-property (string)
  (when (string-match ":property" string)
    (list :property t)))
(add-to-list 'org-transclusion-keyword-value-functions
             #'my/manifolding-atlas-transclusion-keyword-value-property)
...
#+begin_src emacs-lisp
(defun my/org-transclusion-content-format-property (_type content keyword-values)
...
(add-to-list 'org-transclusion-content-format-functions
             #'my/org-transclusion-content-format-property)
...
#+begin_src emacs-lisp
(defun my/manifolding-atlas-db-query-transclude-links-to (target-id)
...
#+begin_src emacs-lisp
(defun my/manifolding-atlas-links--find-transclude-refs (target-id)
