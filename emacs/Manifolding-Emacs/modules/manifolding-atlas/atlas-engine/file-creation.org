...
This is the default value of `manifolding-atlas-find-default-create-fn'."
  (let* ((fname (my/manifolding-atlas-prompt-file-name title nil))
         ...
         (template (my/manifolding-atlas-prompt-template--choose))
         (template-content (my/manifolding-atlas--get-template-content template)))
     ;; Open the newly created file in org-capture so you see title + all WARNING properties immediately
    (let ((org-capture-templates
           `(("n" "New Note" plain (file ,file-path) "" :unnarrowed t :empty-lines 1 :kill-buffer t))))
      (org-capture nil "n")
      ;; Now in capture buffer visiting file-path — WARNING tag already set like TODO, straight to body
...
Uses `consult--read' with live preview when available, otherwise plain `completing-read'."
      ...
      (if (fboundp 'consult--read)
          (consult--read cands
                         ...
                         :state (lambda (action cand)
                                  (pcase action
                                    ('preview
                                     (let ((content (my/manifolding-atlas--get-template-content cand)))
                                       (with-current-buffer (get-buffer-create "*Template Preview*")
                                         (erase-buffer)
                                         (insert (or content (format "No content for %s" (or cand ""))))
                                         (goto-char (point-min))
                                         (display-buffer (current-buffer) '((display-buffer-in-side-window) (side . right) (window-width . 0.5))))))
                                    ('return
                                     (when (get-buffer "*Template Preview*")
                                       (kill-buffer "*Template Preview*"))))))
