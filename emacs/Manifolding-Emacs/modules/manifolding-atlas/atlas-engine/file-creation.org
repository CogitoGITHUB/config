...
Prompts for subdir and file name, then runs the property grid
(collect-prompts-with-fast) and creates the note with whatever tags,
properties, and body it gathers. Any property left unanswered/skipped
defaults to WARNING via the normal missing-prompts mechanism.
...
This is the default value of `manifolding-atlas-find-default-create-fn'."
  (let* ((subdir (my/manifolding-atlas-prompt-subdir))
         (fname (my/manifolding-atlas-prompt-file-name title subdir))
...
(defun my/manifolding-atlas-collect--fallback-choose (contexts)
  ...
  (let* ((cands (my/manifolding-atlas-collect--custom-candidates contexts))
         (picked (let ((enable-recursive-minibuffers t))
                   (completing-read-multiple
