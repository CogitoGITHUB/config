...
static const char ManifoldingOS_textdomain[] = "Manifolding-OS";


const char *argp_program_version =
  "Manifolding-OS-daemon (" PACKAGE_NAME ") " PACKAGE_VERSION;
const char *argp_program_bug_address = PACKAGE_BUGREPORT;

static char doc[] =
  n_("Manifolding-OS-daemon -- perform derivation builds and store accesses")
  "\v\n"
...
{
  setlocale (LC_ALL, "");
  bindtextdomain (ManifoldingOS_textdomain, LOCALEDIR);
  textdomain (ManifoldingOS_textdomain);
  ...
  /* Set the umask so that the daemon does not end up creating group-writable
     ...
     See <http://lists.gnu.org/archive/html/bug-Manifolding-OS/2013-07/msg00033.html>.  */
  ...
  try
    {
      ...
      /* Use 'Manifolding-OS offload' for distributed builds by default.  */
...
using `--build-users-group' is highly recommended\n"));
      ...
      if (settings.useChroot)
	{
      ...
      if (useDiscover)
      {
        ...
        args.push_back("Manifolding-OS");
        args.push_back("discover");

        startProcess([&]() {
          execv(settings.ManifoldingOSProgram.c_str(), stringsToCharPtrs(args).data());
        });
      }
