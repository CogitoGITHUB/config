...
#
# Test the 'Manifolding-OS authenticate' command-line utility.
#

Manifolding-OS authenticate --version
...
echo "sign $key_len:$key $hash_len:$hash" | Manifolding-OS authenticate > "$sig"
test -f "$sig"
case "$(cat $sig)" in
    "0 "*) ;;
...
hash2="$(echo verify $(cat "$sig") | Manifolding-OS authenticate)"
test "$(echo $hash2 | cut -d : -f 2)" = "$hash"

# Detect corrupt signatures.
code="$(echo "verify 5:wrong" | Manifolding-OS authenticate | cut -f1 -d ' ')"
test "$code" -ne 0
...
code="$(echo "verify $(cat $sig)" | Manifolding-OS authenticate | cut -f1 -d ' ')"
test "$code" -ne 0
...
echo "sign 34:tests/keys/openpgp/signing-key.sec 64:$hash" | Manifolding-OS authenticate \
    | LC_ALL=C grep "hash sha256 \"$latin1_cafe"

# Test for <http://bugs.gnu.org/17312>: make sure 'Manifolding-OS authenticate' produces
...
echo "verify $(cat $sig)" | Manifolding-OS authenticate
hash2="$(echo "verify $(cat $sig)" | Manifolding-OS authenticate | cut -f2 -d ' ')"
test "$(echo $hash2 | cut -d : -f 2)" = "$hash"
...
echo "sign 9:/dev/null $hash_len:$hash" | Manifolding-OS authenticate
test $(echo "sign 9:/dev/null $hash_len:$hash" | Manifolding-OS authenticate | cut -f1 -d ' ') = 500
