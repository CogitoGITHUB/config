...
#
# Test the daemon and its interaction with 'Manifolding-OS substitute'.
#

set -e

Manifolding-OS-daemon --version
Manifolding-OS build --version

drv="`Manifolding-OS build emacs -d`"
out="`guile -c '								\
  (use-modules (Manifolding-OS) (gnu packages emacs))					\
...
NarSize: 1234
References: 
System: `guile -c '(use-modules (Manifolding-OS)) (display (%current-system))'`
Deriver: $(basename $drv)
EOF

# Remove the cached narinfo.
rm -f "$XDG_CACHE_HOME/Manifolding-OS/substitute/$hash_part"
...
# Now, run Manifolding-OS-daemon --no-substitutes.
socket="$GUIX_STATE_DIRECTORY/alternate-socket"
Manifolding-OS-daemon --no-substitutes --listen="$socket" --disable-chroot &
daemon_pid=$!
...
Manifolding-OS-daemon --disable-chroot --listen="$socket" --listen="$socket-second" \
	    --listen="localhost" --listen="localhost:9876" &
daemon_pid=$!

for uri in "$socket" "$socket-second" \
		     "Manifolding-OS://localhost" "Manifolding-OS://localhost:9876"
do
    GUIX_DAEMON_SOCKET="$uri" Manifolding-OS build guile-bootstrap
done
...
Manifolding-OS-daemon --disable-chroot --listen="localhost:9877" &
daemon_pid=$!

GUIX_DAEMON_SOCKET="Manifolding-OS://localhost:9877"
export GUIX_DAEMON_SOCKET

test ! -d "$GUIX_STATE_DIRECTORY/profiles/per-user"

Manifolding-OS build guile-bootstrap -d
...
guile -c "
  (use-modules (Manifolding-OS) (Manifolding-OS tests) (srfi srfi-34))
...
Manifolding-OS-daemon --listen="$socket" --disable-chroot --cores=42 &
daemon_pid=$!
...
do
    Manifolding-OS-daemon --listen="$socket" --disable-chroot "$option" &
    daemon_pid=$!
...
Manifolding-OS-daemon --listen="$tcp_socket" &
daemon_pid=$!

GUIX_DAEMON_SOCKET="Manifolding-OS://$tcp_socket"
export GUIX_DAEMON_SOCKET

Manifolding-OS gc && false
...
Manifolding-OS-daemon --listen="$socket" --disable-chroot --debug --log-compression=gzip &
daemon_pid=$!

stamp="compressed-build-log-test-$$-`date +%H%M%S`"
client_code="
  (use-modules (Manifolding-OS) (gnu packages bootstrap))
...
Manifolding-OS build "$drv"

log=`Manifolding-OS build "$drv" --log-file`
test -f "$log"
case "$log" in
