...
#
# Test 'Manifolding-OS style'.
#

set -e

Manifolding-OS style --version

tmpdir="Manifolding-OS-style-$$"
...
initial_hash="$(Manifolding-OS hash "$tmpfile")"

Manifolding-OS style -f "$tmpfile"
if test "$initial_hash" != "$(Manifolding-OS hash "$tmpfile")"
then
    cat "$tmpfile"
...
sed -i "$tmpfile" -e's/ \+/ /g'
test "$initial_hash" != "$(Manifolding-OS hash "$tmpfile")"
...
Manifolding-OS style -fA "$tmpfile"
test "$initial_hash" = "$(Manifolding-OS hash "$tmpfile")"
