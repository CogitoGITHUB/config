...
#
# Test the `Manifolding-OS lint' command-line utility.
#

Manifolding-OS lint --version
...
out=`Manifolding-OS lint -c synopsis,description dummy 2>&1`
test `grep_warning "$out"` -eq 3

out=`Manifolding-OS lint -c synopsis dummy 2>&1`
test `grep_warning "$out"` -eq 2

out=`Manifolding-OS lint -c description dummy 2>&1`
test `grep_warning "$out"` -eq 1

out=`Manifolding-OS lint -c description,synopsis dummy 2>&1`
test `grep_warning "$out"` -eq 3

working_dir="$(pwd)"
cd "$module_dir"
out=`Manifolding-OS lint -c name -f foo.scm 2>&1`
cd "$working_dir"
test `echo "$out" | grep -E -c "breaks from alphabetical order"` -eq 1

Manifolding-OS lint -c synopsis,invalid-checker dummy 2>&1 | \
   grep -q 'invalid-checker: invalid checker'

# Make sure specifying multiple packages works.
Manifolding-OS lint -c inputs-should-be-native dummy dummy@42 dummy


# Use --load-path instead.
unset GUIX_PACKAGE_PATH

out=`Manifolding-OS lint -L $module_dir -c synopsis,description dummy 2>&1`
test `grep_warning "$out"` -eq 3

# Make sure specifying multiple packages works.
Manifolding-OS lint -L $module_dir -c inputs-should-be-native dummy dummy@42 dummy

# Test '-L' with a relative file name.  'Manifolding-OS lint' will see "t-xyz/foo.scm"
...
(cd "$module_dir"/.. ; Manifolding-OS lint -c formatting -L "$(basename "$module_dir")" dummy@42) 2>&1 > "$module_dir/out"
...
(cd "$module_dir"/.. ; Manifolding-OS lint -c synopsis -L "$(basename "$module_dir")" dummy@42) 2>&1 > "$module_dir/out"
