...
#
# Test the 'Manifolding-OS pack --relocatable' using the external store, if any.
#

Manifolding-OS pack --version

# 'Manifolding-OS pack --relocatable' requires a C compiler and libc.a, which our
...
then
    ...
    tarball="`Manifolding-OS pack -R -S /Bin=bin sed`"
...
case "`uname -m`" in
    x86_64|i?86)
	# Try '-RR' and PRoot.
	tarball="`Manifolding-OS pack -RR -S /Bin=bin sed`"
	...
	then
	    ...
	    tarball="`Manifolding-OS pack -RR -S /bin=bin bash-minimal`"
	    (cd "$test_directory"; tar xf "$tarball")
	    ...
	    do
		for i in $(Manifolding-OS gc -R $(Manifolding-OS build bash-minimal | grep -v -e '-doc$'))
		do
...
EOF
    ...
    tarball="$(Manifolding-OS pack -S /bin=bin -R -m "$test_directory/manifest.scm")"
    (cd "$test_directory"; tar xf "$tarball")
...
tarball="`Manifolding-OS pack -R -S /share=share groff:doc`"
(cd "$test_directory"; tar xf "$tarball")
...
Manifolding-OS pack -RR -e '(@ (gnu packages python-xyz) python-numpy)' python-scipy --no-grafts -n

# Check that packages that mix executable and support files (e.g. git) in the
# "binary" directories still work after wrapped.
cat >"$test_directory/manifest.scm" <<'EOF'
(use-modules (Manifolding-OS) (Manifolding-OS profiles) (Manifolding-OS search-paths)
 ...
 (list (manifest-entry
        ...
        (search-paths
         (list (search-path-specification
                ...
                (separator #f)))))))
EOF
tarball="`Manifolding-OS pack -RR -S /opt= -m $test_directory/manifest.scm`"
(cd "$test_directory"; tar xvf "$tarball")
chmod +w "$test_directory"
