...
#
# Test the 'Manifolding-OS refresh' command-line utility.
#

Manifolding-OS refresh --version

manifest="t-Manifolding-OS-refresh-manifest-$$.scm"
module_dir="t-Manifolding-OS-refresh-modules-$$"
trap 'rm -f "$manifest"; rm -rf "$module_dir"' EXIT

# Tell the 'test' updater what to simulate.
export GUIX_TEST_UPDATER_TARGETS
idutils_version="$(Manifolding-OS package -A ^idutils$ | cut -f2)"
...
Manifolding-OS refresh -t test idutils	# XXX: should return non-zero?
case "$(Manifolding-OS refresh -t test idutils 2>&1)" in
    *"$idutils_version"*"already the latest version"*) true;;
    *) false;;
esac

# No-op when updating to same version.
case "$(Manifolding-OS refresh -t test -u idutils \
...
Manifolding-OS refresh -t test libreoffice # XXX: should return non-zero?
case "$(Manifolding-OS refresh -t test libreoffice 2>&1)" in
...
for spec in "guile"					\
	    ...
	    "-s_core"
do
    Manifolding-OS refresh -t test $spec
    case "$(Manifolding-OS refresh -t test $spec 2>&1)" in
...
EOF
Manifolding-OS refresh -t test -L "$module_dir" the-test-package
Manifolding-OS refresh -t test -L "$module_dir" the-test-package -u \
     --keyring="$module_dir/keyring.kbx"  # so we don't create $HOME/.config
grep 'version "5.5"' "$module_dir/sample.scm"
grep "$(Manifolding-OS hash -H sha256 -f nix-base32 "$module_dir/source")" "$module_dir/sample.scm"
grep '(inputs (list grep sed))' "$module_dir/sample.scm"

# Specifying a target version.
Manifolding-OS refresh -t test guile=2.0.0 # XXX: should return non-zero?
case "$(Manifolding-OS refresh -t test guile=2.0.0 2>&1)" in
    *"failed to find"*"2.0.0"*) true;;
    *) false;;
esac

Manifolding-OS refresh -t test guile --target-version=2.0.0 # XXX: should return non-zero?
case "$(Manifolding-OS refresh -t test guile --target-version=2.0.0 2>&1)" in
    *"failed to find"*"2.0.0"*) true;;
    *) false;;
esac
...
case "$(Manifolding-OS refresh -t test guile --target-version=3 2>&1)" in
    *"would be upgraded"*"3.13.3"*) true;;
    *) false;;
esac

# Partial spec version => select the newest release prefixed by it.
case "$(Manifolding-OS refresh -t test guile=~3 2>&1)" in
    *"would be upgraded"*"3.13.3"*) true;;
    *) false;;
esac
...
Manifolding-OS refresh --list-updaters
