...
#
# Test the `Manifolding-OS download' command-line utility.
#

Manifolding-OS download --version

# Make sure it fails here.
Manifolding-OS download http://does.not/exist && false

Manifolding-OS download unknown://some/where && false

Manifolding-OS download /does-not-exist && false

# This one should succeed.
Manifolding-OS download "file://$abs_top_srcdir/README"

# And this one, without the URI scheme.
Manifolding-OS download "$abs_top_srcdir/README"

# This one too, even if it cannot talk to the daemon.
GUIX_DAEMON_SOCKET="/nowhere" Manifolding-OS download -o "$output" \
		  "file://$abs_top_srcdir/README"
cmp "$output" "$abs_top_srcdir/README"

# This one should fail.
Manifolding-OS download "file:///does-not-exist" "file://$abs_top_srcdir/README" && false
...
# We expect that Manifolding-OS hash is working properly or at least that the output of
# 'Manifolding-OS download' is consistent with 'Manifolding-OS hash'.
expected_hash=$(Manifolding-OS hash -rx $test_git_repo)

# Test the different options
for option in "" "--commit=$commit" "--commit=v1" "--branch=master"
do
    command_output="$(Manifolding-OS download --git $option "file://$test_git_repo")"
...
# Should fail.
Manifolding-OS download --git --branch=non_existent "file://$test_git_repo" && false

# Same but download to file instead of store.
Manifolding-OS download --git "file://$test_git_repo" -o $output_dir
