...
#
# Test the 'Manifolding-OS git authenticate' command-line utility.
#
...
# Keep in sync with '%default-channels' in (Manifolding-OS channels)!
...
Manifolding-OS git authenticate "$intro_commit" "$intro_signer"	\
...
Manifolding-OS git authenticate				\
     --cache-key="$cache_key" --stats		\
     --end="$v1_2_0_commit"
...
rm "$XDG_CACHE_HOME/Manifolding-OS/authentication/$cache_key"
...
# This should fail because these commits lack '.Manifolding-OS-authorizations'.
Manifolding-OS git authenticate "$v1_0_0_commit" "$v1_0_0_signer" \
