...
#
# Test the 'Manifolding-OS locate' command-line utility.
#
...
Manifolding-OS locate --version

# Preparing db locations for both indexation methods.
mkdir -p "$(dirname "$tmpdb_manifests")" "$(dirname "$tmpdb_store")"

cmd_manifests="Manifolding-OS locate --database=$tmpdb_manifests --method=manifests"
cmd_store="Manifolding-OS locate --database=$tmpdb_store --method=store"

# Lookup without any db should fail.
Manifolding-OS locate --database="$tmpdb_manifests" guile && false
Manifolding-OS locate --database="$tmpdb_store" guile && false

# Lookup without anything in db should yield no results because the indexer
# didn't stumble upon any profile.
test -z "$(Manifolding-OS locate --database="$tmpdb_manifests" guile)"

# Install a package.
Manifolding-OS package --bootstrap --install guile-bootstrap \
     --profile="$tmpdir/profile"

# Look for 'guile'.
$cmd_manifests --update
$cmd_manifests guile | grep "$(Manifolding-OS build guile-bootstrap)/bin/guile"
...
then
    $cmd_store --update
    $cmd_store guile
    $cmd_store guile | grep "$(Manifolding-OS build guile-bootstrap)/bin/guile"
    $cmd_store boot-9.scm | grep ^guile-bootstrap
fi

# The command below is an error: "no files to search for"...
Manifolding-OS locate && false

# ... but this one is fine.
Manifolding-OS locate --clear
