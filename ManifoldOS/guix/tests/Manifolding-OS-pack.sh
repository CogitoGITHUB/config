...
#
# Test the `Manifolding-OS pack' command-line utility.
#
...
Manifolding-OS pack intelmetool -s armhf-linux -n && false

# Compute the derivation of a pack.
drv="`Manifolding-OS pack coreutils -d --no-grafts`"
Manifolding-OS gc -R "$drv" | grep "`Manifolding-OS build coreutils -d --no-grafts`"
...
drv="`Manifolding-OS pack idutils -d --no-grafts --target=arm-linux-gnueabihf`"
Manifolding-OS gc -R "$drv" | \
    grep "`Manifolding-OS build idutils --target=arm-linux-gnueabihf -d --no-grafts`"
Manifolding-OS gc -R "$drv" | grep "`Manifolding-OS build idutils -d --no-grafts`" && false

# Build a tarball with no compression.
Manifolding-OS pack --compression=none --bootstrap guile-bootstrap

# Build a tarball (with compression).  Check that '-e' works as well.
out1="`Manifolding-OS pack --bootstrap guile-bootstrap`"
out2="`Manifolding-OS pack --bootstrap -e '(@ (gnu packages bootstrap) %bootstrap-guile)'`"
test -n "$out1"
test "$out1" = "$out2"

# Test '--root'.
Manifolding-OS pack -r "$test_directory/my-guile" --bootstrap guile-bootstrap
test "`readlink "$test_directory/my-guile"`" = "$out1"
Manifolding-OS gc --list-roots | grep "^$test_directory/my-guile$"
rm "$test_directory/my-guile"

# Build a tarball with a symlink.
the_pack="`Manifolding-OS pack --bootstrap -S /opt/gnu/bin=bin guile-bootstrap`"
...
Manifolding-OS pack --dry-run --bootstrap -f docker guile-bootstrap

# Build a Docker image with a symlink.
Manifolding-OS pack --dry-run --bootstrap -f docker -S /opt/gnu= guile-bootstrap

# Build a tarball pack of cross-compiled software.  Use coreutils because
# guile-bootstrap is not intended to be cross-compiled.
Manifolding-OS pack --dry-run --bootstrap --target=arm-linux-gnueabihf coreutils

# Likewise, 'Manifolding-OS pack -R' requires a full-blown toolchain (because
# 'glibc-bootstrap' lacks 'libc.a'), hence '--dry-run'.
Manifolding-OS pack -R --dry-run --bootstrap -S /mybin=bin guile-bootstrap
...
drv1="`Manifolding-OS pack --no-grafts -n guile 2>&1 | grep pack.*\.drv`"
drv2="`Manifolding-OS pack --no-grafts -n --with-source=guile=$test_directory guile 2>&1 | grep pack.*\.drv`"
test -n "$drv1"
test "$drv1" != "$drv2"
...
EOF
drv="`Manifolding-OS pack --no-grafts -d -m "$test_directory/manifest1.scm" -m "$test_directory/manifest2.scm"`"
Manifolding-OS gc -R "$drv" | grep `Manifolding-OS build guile -d --no-grafts`
Manifolding-OS gc -R "$drv" | grep `Manifolding-OS build emacs -d --no-grafts`
