...
#
# Test 'Manifolding-OS environment'.
#

set -e

Manifolding-OS environment --version
...
tmpdir="t-Manifolding-OS-environment-$$"
trap 'rm -r "$tmpdir"' EXIT

mkdir "$tmpdir"

# Make sure the exit value is preserved.
if Manifolding-OS environment --container --ad-hoc --bootstrap guile-bootstrap \
        -- guile -c '(exit 42)'
then
    false
else
...
root="$tmpdir/root"
Manifolding-OS environment -C --ad-hoc --bootstrap guile-bootstrap -r "$root" -- guile --version
Manifolding-OS environment -C -p "$root" --bootstrap -- guile --version
path1=$(Manifolding-OS environment -C -p "$root" --bootstrap -- guile -c '(display (getenv "PATH"))')
path2=$(Manifolding-OS environment -C --ad-hoc --bootstrap guile-bootstrap  -- guile -c '(display (getenv "PATH"))')
test "$path1" = "$path2"

# Make sure "localhost" resolves.
Manifolding-OS environment --container --ad-hoc --bootstrap guile-bootstrap \
...
result="`FOOBAR=42; export FOOBAR; Manifolding-OS environment -C --ad-hoc --bootstrap \
   guile-bootstrap -E ^FOO -- guile -c '(display (getenv \"FOOBAR\"))'`"
test "$result" = "42"

# By default, the UID inside the container should be the same as outside.
uid="`id -u`"
inner_uid="`Manifolding-OS environment -C --ad-hoc --bootstrap guile-bootstrap \
...
export tmpdir
inner_uid="`HOME=$tmpdir Manifolding-OS environment -C --ad-hoc --bootstrap guile-bootstrap \
  --user=gnu-Manifolding-OS -- guile -c '(display (getuid))'`"
test $inner_uid = 1000

if test "x$USER" = "x"; then USER="`id -un`"; fi

# Check whether /etc/passwd and /etc/group are valid.
Manifolding-OS environment -C --ad-hoc --bootstrap guile-bootstrap \
...
grep $(Manifolding-OS build guile-bootstrap) $tmpdir/mounts
grep -e "$NIX_STORE_DIR/.*-bash" $tmpdir/mounts # bootstrap bash

rm $tmpdir/mounts

# Make sure 'GUIX_ENVIRONMENT' is set to '~/.Manifolding-OS-profile' when requested
# within a container.
(
  linktest='
(exit (and (string=? (getenv "GUIX_ENVIRONMENT")
                     (string-append (getenv "HOME") "/.Manifolding-OS-profile"))
           (string-prefix? "'"$NIX_STORE_DIR"'"
                           (readlink (string-append (getenv "HOME")
                                                    "/.Manifolding-OS-profile")))))'

  cd "$tmpdir" \
     && Manifolding-OS environment --bootstrap --container --link-profile \
...
touch "$tmpdir/umock"
HOME="$tmpdir" Manifolding-OS environment --bootstrap --container --user=foognu \
     --ad-hoc guile-bootstrap --pure \
     --share="$tmpdir/umock" \
     -- guile -c "$usertest"
...
(
  cd "$tmpdir" \
    && Manifolding-OS environment --bootstrap --container --no-cwd --user=foo  \
            --ad-hoc guile-bootstrap --pure \
...
(
  cd "$tmpdir" \
    && Manifolding-OS environment --bootstrap --container --no-cwd --cwd=/tmp \
            --ad-hoc guile-bootstrap --pure \
...
(
  cd "$tmpdir/home/wd" \
    && HOME="$home_dir" Manifolding-OS environment --bootstrap --container --user=foo \
            --cwd=. --ad-hoc guile-bootstrap --pure \
            -- /bin/sh -c 'test "$(pwd)" = "/home/foo/wd"'
)

# Check that the root file system is read-only by default...
Manifolding-OS environment --bootstrap --container --ad-hoc guile-bootstrap \
...
if Manifolding-OS environment --bootstrap --container \
...
Manifolding-OS shell -C --emulate-fhs --bootstrap guile-bootstrap \
...
Manifolding-OS shell -CF --bootstrap guile-bootstrap \
...
Manifolding-OS shell -CF --bootstrap guile-bootstrap glibc \
...
Manifolding-OS shell -CF coreutils -- env | grep ^PATH=/bin:/usr/bin:/sbin:/usr/sbin.*
...
PATH=/foo $(type -P Manifolding-OS) shell -CF -E ^PATH$ coreutils \
     -- env | grep ^PATH=.*:/foo

# '--symlink' works.
echo "TESTING SYMLINK IN CONTAINER"
Manifolding-OS shell --bootstrap guile-bootstrap --container \
     --symlink=/usr/bin/guile=bin/guile -- \
     /usr/bin/guile --version

# A dangling symlink causes the command to fail.
Manifolding-OS shell --bootstrap -CS /usr/bin/python=bin/python guile-bootstrap -- exit && false

# An invalid symlink spec causes the command to fail.
Manifolding-OS shell --bootstrap -CS bin/guile=/usr/bin/guile guile-bootstrap -- exit && false

# Check whether '--nesting' works.
Manifolding-OS build hello -d
env="$(type -P pre-inst-env)"
Manifolding-OS shell -C -D Manifolding-OS -- "$env" Manifolding-OS build hello -d && false # cannot work
hello_drv="$(Manifolding-OS build hello -d)"
hello_drv_nested="$(cd "$(dirname env)" && Manifolding-OS shell --bootstrap -E GUIX_BUILD_OPTIONS -CW -D Manifolding-OS -- "$env" Manifolding-OS build hello -d)"
