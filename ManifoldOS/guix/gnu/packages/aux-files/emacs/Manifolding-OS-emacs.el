;;; -*- lexical-binding: t; -*-
;;; Manifolding-OS-emacs.el --- Emacs packages installed with Guix
...
(defvar Manifolding-OS-emacs-autoloads-regexp
...
(defcustom Manifolding-OS-emacs-verbose nil
  "Set to true to provide verbose messages, such as when loading packages."
  :type 'boolean
  :group 'Manifolding-OS-emacs)

(defun Manifolding-OS-emacs-find-autoloads (directory)
  ...
  (if (file-directory-p directory)
      (delete-dups (mapcar #'file-name-sans-extension
                           (directory-files directory 'full-name
                                            Manifolding-OS-emacs-autoloads-regexp)))
    (when Manifolding-OS-emacs-verbose
      (message "Ignoring nonexistent load-path directory: %s" directory)
      nil)))

(defun Manifolding-OS-emacs--load-file-no-error (file)
  "Load FILE, ignoring any errors."
  (load file 'noerror (not Manifolding-OS-emacs-verbose)))

(defsubst Manifolding-OS-emacs--non-core-load-path ()
  "Filter out core Elisp directories, which are already handled by Emacs."
  (seq-filter (lambda (dir)
                (string-match-p "/share/emacs/site-lisp" dir))
              load-path))

(defun Manifolding-OS-emacs--subdirs-files ()
  "Return the Guix subdirs.el files found on the (non-core) load path."
  (seq-filter #'file-exists-p
              (mapcar (lambda (dir)
                        (expand-file-name "subdirs.el" dir))
                      (Manifolding-OS-emacs--non-core-load-path))))

;;;###autoload
(defun Manifolding-OS-emacs-autoload-packages (&optional no-reload)
  "Load autoload definitions for packages in `load-path'.

Autoload definitions are contained within files matching
`Manifolding-OS-emacs-autoload-regexp'; load each file matching that regexp.
...
found in `load-path' to discover newly installed packages."
  ...
  (unless no-reload
    (mapc #'Manifolding-OS-emacs--load-file-no-error (Manifolding-OS-emacs--subdirs-files))
    (setq load-path (delete-dups load-path)))

  (let ((autoloads (mapcan #'Manifolding-OS-emacs-find-autoloads
                           (Manifolding-OS-emacs--non-core-load-path))))
    (mapc #'Manifolding-OS-emacs--load-file-no-error autoloads)))

;;;###autoload
(defun Manifolding-OS-emacs-load-package-descriptors ()
  "Load descriptors for packages found in EMACSLOADPATH via subdirs.el."
  (dolist (subdirs-file (Manifolding-OS-emacs--subdirs-files))
    (with-temp-buffer
...
(provide 'Manifolding-OS-emacs)

;;; Manifolding-OS-emacs.el ends here
