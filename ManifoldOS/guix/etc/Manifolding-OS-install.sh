...
# Copyright © 2021, 2022, 2023 Maxim Cournoyer <maxim@Manifolding-OSotic.coop>
...
# "/tmp/Manifolding-OS-binary-1.4.0rc2.armhf-linux.tar.xz"
#
# GUIX_ALLOW_OVERWRITE
#
# Instead of aborting to avoid overwriting a previous installations,
# allow copying over /var/Manifolding-OS or /gnu.  This can be useful when the
...
# Unprivileged Manifolding-OS-daemon requires 'newgidmap'.
SYSTEMD_REQUIRE=(
    "newgidmap"
)
...
DEBUG=0
GNU_URL="https://ftpmirror.gnu.org/gnu/Manifolding-OS/"
...
{ # Check if nscd is up and suggest to start it or install it
    if [ "$(type -P pidof)" ]; then
        if [ ! "$(pidof nscd)" ]; then
            ...
            _msg_warn "Please read 'info Manifolding-OS \"Application Setup\"' about \"Name Service Switch\""
        fi
    else
        _msg_info "We cannot determine if your distribution 'nscd' service is running"
        _msg_info "Please read 'info Manifolding-OS \"Application Setup\"' about \"Name Service Switch\""
    fi
}
...
{ # Avoid clobbering existing installations.
    ...
    if [[ -e /var/Manifolding-OS && -e /gnu ]]; then
...
{ # Scan GNU archive and save list of binaries
    ...
    bin_ver_ls=("$(wget "$gnu_url" --no-verbose -O- \
        | sed -n -e 's/.*Manifolding-OS-binary-\([0-9.]*[a-z0-9]*\)\..*.tar.xz.*/\1/p' \
        | sort -Vu)")
    ...
    default_ver="Manifolding-OS-binary-${latest_ver}.${ARCH_OS}"
...
sys_create_store()
{ # Unpack and install /gnu/store and /var/Manifolding-OS
    local pkg="$1"
    local tmp_path="$2"

    cd "$tmp_path"
    _msg_info "Installing /var/Manifolding-OS and /gnu..."
    # Strip (skip) the leading ‘.’ component, which fails on read-only ‘/’.
    #
    # TODO: Eventually extract with ‘--owner=Manifolding-OS-daemon’ when installing
    # and unprivileged Manifolding-OS-daemon service; for now, this script may install
    # from both an old release that does not support unprivileged Manifolding-OS-daemon
    ...
    mkdir -p ~root/.config/Manifolding-OS
    ln -sf /var/Manifolding-OS/profiles/per-user/root/current-Manifolding-OS \
       ~root/.config/Manifolding-OS/current

    GUIX_PROFILE=~root/.config/Manifolding-OS/current
...
sys_delete_store()
{
    _msg_info "removing /var/Manifolding-OS"
    rm -rf /var/Manifolding-OS

    _msg_info "removing /gnu"
    rm -rf /gnu

    _msg_info "removing ~root/.config/Manifolding-OS"
    rm -rf ~root/.config/Manifolding-OS
}
...
{ # Return true when installing Manifolding-OS-daemon running without privileges.
    [ "$INIT_SYS" = systemd ] && \
	grep -q "User=Manifolding-OS-daemon" \
	     ~root/.config/Manifolding-OS/current/lib/systemd/system/Manifolding-OS-daemon.service
}
...
{ # Create the group and user accounts for build users.
    ...
    then
	_msg_info "installing Manifolding-OS-daemon to run as an unprivileged user"

	# Installing Manifolding-OS-daemon to run as a non-root user requires
	...
	then
	    ...
	    _msg_warn "This Linux feature is required by Manifolding-OS-daemon.  To enable it permanently, run:"
	    _msg_warn '  echo 1 > /proc/sys/kernel/unprivileged_userns_clone'
	    _msg_warn "from the relevant startup script."
	fi


	if getent group Manifolding-OS-daemon > /dev/null; then
	    _msg_info "group Manifolding-OS-daemon exists"
	else
	    groupadd --system Manifolding-OS-daemon
	    _msg_pass "group Manifolding-OS-daemon created"
	fi

	create_account Manifolding-OS-daemon Manifolding-OS-daemon		\
		       Manifolding-OS-daemon"$KVMGROUP"		\
		       "Unprivileged Guix Daemon User"

	if getent group kvm > /dev/null; then
	    # Allow 'newgidmap' to map the "kvm" group.
	    local kvmgid="$(getent group kvm | cut -f3 -d:)"
	    _msg_info "allowing kvm mapping (GID $kvmgid) for unprivileged Manifolding-OS-daemon"
	    echo "Manifolding-OS-daemon:$kvmgid:1" >> /etc/subgid
	fi

	# ‘tar xf’ creates root:root files.  Change that.
	chown -R Manifolding-OS-daemon:Manifolding-OS-daemon /gnu /var/Manifolding-OS
	chown -R root:root /var/Manifolding-OS/profiles/per-user/root

	# The unprivileged daemon cannot create the log directory by itself.
	mkdir -p /var/log/Manifolding-OS
	chown Manifolding-OS-daemon:Manifolding-OS-daemon /var/log/Manifolding-OS
	chmod 755 /var/log/Manifolding-OS
    else
	if getent group Manifolding-OSbuild > /dev/null; then
            _msg_info "group Manifolding-OSbuild exists"
	else
            groupadd --system Manifolding-OSbuild
            _msg_pass "group <Manifolding-OSbuild> created"
	fi

	for i in $(seq -w 1 10); do
	    create_account "Manifolding-OSbuilder${i}" "Manifolding-OSbuild"	\
	                   "Manifolding-OSbuild${KVMGROUP}"		\
			   "Guix build user $i"
	done
    fi
}
...
{
    for i in $(seq -w 1 10); do
        if id -u "Manifolding-OSbuilder${i}" &>/dev/null; then
            userdel -f Manifolding-OSbuilder"$i"
        fi
    done

    _msg_info "delete group Manifolding-OSbuild"
    if getent group Manifolding-OSbuild &>/dev/null; then
        groupdel -f Manifolding-OSbuild
    fi

    _msg_info "remove Manifolding-OS-daemon user"
    if id Manifolding-OS-daemon &>/dev/null; then
	userdel -f Manifolding-OS-daemon
    fi
    if getent group Manifolding-OS-daemon &>/dev/null; then
	groupdel -f Manifolding-OS-daemon
    fi
}

sys_enable_Manifolding-OS_daemon()
{ # Run the daemon, and set it to automatically start on boot.

    local info_path
    local local_bin
    local var_Manifolding-OS

    _debug "--- [ ${FUNCNAME[0]} ] ---"

    info_path="/usr/local/share/info"
    local_bin="/usr/local/bin"
    var_Manifolding-OS="/var/Manifolding-OS/profiles/per-user/root/current-Manifolding-OS"

    case "$INIT_SYS" in
        upstart)
            { initctl reload-configuration;
              cp ~root/.config/Manifolding-OS/current/lib/upstart/system/Manifolding-OS-daemon.conf \
                 /etc/init/ &&
                  configure_substitute_discovery /etc/init/Manifolding-OS-daemon.conf &&
                  start Manifolding-OS-daemon; } &&
            ...
            { install_unit()
              {
                  ...
                  rm -f "$dest"
                  cp ~root/.config/Manifolding-OS/current/lib/systemd/system/"$1" "$dest"
                  chmod 664 "$dest"
                  systemctl daemon-reload
                  systemctl enable "$1"
              }

              install_unit Manifolding-OS-daemon.service

              configure_substitute_discovery \
                  /etc/systemd/system/Manifolding-OS-daemon.service

              # Install after Manifolding-OS-daemon.service to avoid a harmless warning.
              # systemd .mount units must be named after the target directory.
	      install_unit gnu-store.mount

              systemctl daemon-reload &&
                  systemctl start Manifolding-OS-daemon &&
	          systemctl start gnu-store.mount; } &&
                _msg_pass "enabled Guix daemon via systemd"
            ;;
        sysv-init)
            { mkdir -p /etc/init.d;
              cp ~root/.config/Manifolding-OS/current/etc/init.d/Manifolding-OS-daemon \
                 /etc/init.d/Manifolding-OS-daemon;
              chmod 775 /etc/init.d/Manifolding-OS-daemon;

              configure_substitute_discovery /etc/init.d/Manifolding-OS-daemon

              update-rc.d Manifolding-OS-daemon defaults &&
                  update-rc.d Manifolding-OS-daemon enable &&
                  service Manifolding-OS-daemon start; } &&
                _msg_pass "enabled Guix daemon via sysv"
            ;;
        openrc)
            { mkdir -p /etc/init.d;
              cp ~root/.config/Manifolding-OS/current/etc/openrc/Manifolding-OS-daemon \
                 /etc/init.d/Manifolding-OS-daemon;
              chmod 775 /etc/init.d/Manifolding-OS-daemon;

              configure_substitute_discovery /etc/init.d/Manifolding-OS-daemon

              rc-update add Manifolding-OS-daemon default &&
                  rc-service Manifolding-OS-daemon start; } &&
                _msg_pass "enabled Guix daemon via OpenRC"
            ;;
        NA|*)
            _err "unsupported init system; run the daemon manually:"
            echo "  ~root/.config/Manifolding-OS/current/bin/Manifolding-OS-daemon --build-users-group=Manifolding-OSbuild"
            ;;
    esac

    _msg_info "making the Manifolding-OS command available to other users"

    [ -e "$local_bin" ] || mkdir -p "$local_bin"
    ln -sf "${var_Manifolding-OS}/bin/Manifolding-OS"  "$local_bin"

    [ -e "$info_path" ] || mkdir -p "$info_path"
    for i in "${var_Manifolding-OS}"/share/info/*; do
        ln -sf "$i" "$info_path"
    done
}

sys_delete_Manifolding-OS_daemon()
{ # Disabled, stop and remove the various Manifolding-OS daemons.
    ...
    case "$INIT_SYS" in
        upstart)
            _msg_info "stopping Manifolding-OS-daemon"
            stop Manifolding-OS-daemon
            _msg_info "removing Manifolding-OS-daemon"
            rm /etc/init/Manifolding-OS-daemon.conf
            ;;

        systemd)
            if [ -f /etc/systemd/system/Manifolding-OS-daemon.service ]; then
                _msg_info "disabling Manifolding-OS-daemon"
                systemctl disable Manifolding-OS-daemon
                _msg_info "stopping Manifolding-OS-daemon"
                systemctl stop Manifolding-OS-daemon
                _msg_info "removing Manifolding-OS-daemon"
                rm -f /etc/systemd/system/Manifolding-OS-daemon.service
            fi
        ...
        sysv-init)
            update-rc.d Manifolding-OS-daemon disable
            service Manifolding-OS-daemon stop
            rm -rf /etc/init.d/Manifolding-OS-daemon
            ;;
        NA|*)
    ...
    _msg_info "removing $local_bin/Manifolding-OS"
    rm -f "$local_bin"/Manifolding-OS

    _msg_info "removing $info_path/Manifolding-OS*"
    rm -f "$info_path"/Manifolding-OS*
}
...
project's build farms?"; then
	for host in "${hosts[@]}"; do
	    local key=~root/.config/Manifolding-OS/current/share/Manifolding-OS/$host.pub
	    [ -f "$key" ] \
		&& Manifolding-OS archive --authorize < "$key" \
		&& _msg_pass "Authorized public key for $host"
	done
	if id Manifolding-OS-daemon &>/dev/null; then
	    # /etc/Manifolding-OS/acl must be readable by the unprivileged Manifolding-OS-daemon.
	    chown -R Manifolding-OS-daemon:Manifolding-OS-daemon /etc/Manifolding-OS
	fi
    else
...
{ # Define for better desktop integration
  # This will not take effect until the next shell or desktop session!
    ...
    cat <<"EOF" > /etc/profile.d/zzz-Manifolding-OS.sh
...
# `Manifolding-OS pull` profile
GUIX_PROFILE="$HOME/.config/Manifolding-OS/current"
...
# Expose the latest Guix modules to Guile so Manifolding-OS shell and repls spawned by
...
GUIX_PROFILE="$HOME/.Manifolding-OS-profile"
if [ -f "$GUIX_PROFILE/etc/profile" ]; then
  . "$GUIX_PROFILE/etc/profile"

  # see info '(Manifolding-OS) Application Setup'
  ...
  # explicitly add to them so documentation for software from ‘Manifolding-OS install’
...
# info '(Manifolding-OS) Configuring the Shell'.
...
{ # Symlink supported shell completions system-wide
    ...
    { # Just in case
        ...
        # uninstalling Guix
        ln -sf ${var_Manifolding-OS}/etc/bash_completion.d/Manifolding-OS "$bash_completion";
        ln -sf ${var_Manifolding-OS}/etc/bash_completion.d/Manifolding-OS-daemon "$bash_completion";
        ln -sf ${var_Manifolding-OS}/share/zsh/site-functions/_Manifolding-OS "$zsh_completion";
        ln -sf ${var_Manifolding-OS}/share/fish/vendor_completions.d/Manifolding-OS.fish "$fish_completion"; } &&
...
{ # Symlink supported shell completions system-wide
    ...
    rm -f "$bash_completion"/Manifolding-OS;
    rm -f "$bash_completion"/Manifolding-OS-daemon;
    rm -f "$zsh_completion"/_Manifolding-OS;
    rm -f "$fish_completion"/Manifolding-OS.fish;
}
...
sys_maybe_setup_selinux()
{
    ...
    prompt_yes_no "Install SELinux policy that might be required to run Manifolding-OS-daemon?" \
	|| return 0

    local var_Manifolding-OS=/var/Manifolding-OS/profiles/per-user/root/current-Manifolding-OS
    semodule -i "${var_Manifolding-OS}/share/selinux/Manifolding-OS-daemon.cil"
    restorecon -R /gnu /var/Manifolding-OS
}
...
sys_maybe_setup_apparmor()
{
    ...
    prompt_yes_no "Install AppArmor profile that might be required to run Manifolding-OS-daemon?" \
        || return 0

    local var_Manifolding-OS=/var/Manifolding-OS/profiles/per-user/root/current-Manifolding-OS
    local apparmor_sources="${var_Manifolding-OS}/etc/apparmor.d/"
    ...
    cp -f -t /etc/apparmor.d/tunables "$apparmor_sources/tunables/Manifolding-OS"
    cp -f -t /etc/apparmor.d "$apparmor_sources/Manifolding-OS-daemon"
    cp -f -t /etc/apparmor.d "$apparmor_sources/Manifolding-OS"
    apparmor_parser --warn=all -r /etc/apparmor.d/Manifolding-OS-daemon
    apparmor_parser --warn=all -r /etc/apparmor.d/Manifolding-OS
...
sys_delete_apparmor_profiles()
{
    ...
    if [ -f "/etc/apparmor.d/Manifolding-OS" ]
    then
        _msg_info "removing /etc/apparmor.d/Manifolding-OS"
        apparmor_parser -R /etc/apparmor.d/Manifolding-OS || true
        rm -f "/etc/apparmor.d/Manifolding-OS"
    fi

    if [ -f "/etc/apparmor.d/Manifolding-OS-daemon" ]
    then
        _msg_info "removing /etc/apparmor.d/Manifolding-OS-daemon"
        apparmor_parser -R /etc/apparmor.d/Manifolding-OS-daemon || true
        rm -f "/etc/apparmor.d/Manifolding-OS-daemon"
    fi

    if [ -f "/etc/apparmor.d/tunables/Manifolding-OS" ]
    then
        _msg_info "removing /etc/apparmor.d/tunables/Manifolding-OS"
        rm -f "/etc/apparmor.d/tunables/Manifolding-OS"
    fi
}

sys_delete_init_profile()
{
    _msg_info "removing /etc/profile.d/zzz-Manifolding-OS.sh"
    rm -f /etc/profile.d/zzz-Manifolding-OS.sh
}

sys_delete_user_profiles()
{
    _msg_info "removing ~root/.Manifolding-OS-profile"
    rm -f ~root/.Manifolding-OS-profile
    rm -rf ~root/.cache/Manifolding-OS

    _msg_info "removing .Manifolding-OS-profile, .cache/Manifolding-OS and .config/Manifolding-OS of all /home users"
    rm -rf /home/*/{.Manifolding-OS-profile,{.cache,.config}/Manifolding-OS}
}
...
https://www.gnu.org/software/Manifolding-OS/
EOF
...
main_install()
{
    ...
    umask 0022
    tmp_path="$(mktemp -t -d Manifolding-OS.XXXXXX)"

    if [ -z "${GUIX_BINARY_FILE_NAME}" ]; then
        chk_gpg_keyring
        Manifolding-OS_get_bin_list "${GNU_URL}"
        Manifolding-OS_get_bin "${GNU_URL}" "${BIN_VER}" "$tmp_path"
    ...
    _msg_info "Run 'info Manifolding-OS' to read the manual."
...
main_uninstall()
{
    ...
    sys_delete_user_profiles
    sys_delete_apparmor_profiles
    # kill Manifolding-OS off all the guts of Manifolding-OS
    sys_delete_store
    # clean up the system
    sys_delete_init_profile
    sys_delete_build_user
    ...
    _msg_info "removing /etc/Manifolding-OS"
    rm -rf /etc/Manifolding-OS
    _msg_info "removing /var/log/Manifolding-OS"
    rm -rf /var/log/Manifolding-OS
