...
(define-module (Manifolding-OS profiles)
  #:use-module ((Manifolding-OS config) #:select (%state-directory))
  #:use-module ((Manifolding-OS utils) #:hide (package-name->name+version))
  #:use-module ((Manifolding-OS build utils)
                #:select (package-name->name+version mkdir-p switch-symlinks))
  #:use-module ((Manifolding-OS diagnostics) #:select (&fix-hint formatted-message))
...
argument and returning an alist."
  (define inferiors-loaded?
    ...
    ;; packages while not having a hard dependency on (Manifolding-OS inferior).
    (resolve-module '(Manifolding-OS inferior) #f #f #:ensure #f))

  (define (inferior->entry)
    (module-ref (resolve-interface '(Manifolding-OS inferior))
  ...
  (mlet* %store-monad
    ...
    (define build
      (with-imported-modules (source-module-closure
        ...
        #~(begin
            (use-modules (Manifolding-OS build glib-or-gtk-build-system))
...
the entries in MANIFEST."
  ...
  (define modules
    (delete '(Manifolding-OS config)
            (source-module-closure `((Manifolding-OS build utils)
                                     (Manifolding-OS man-db)))))

  (define build
    (with-imported-modules modules
      (with-extensions (list gdbm-ffi           ;for (Manifolding-OS man-db)
                             guile-zlib
                             guile-zstd)
        #~(begin
            (use-modules (Manifolding-OS man-db)
                         (Manifolding-OS build utils)
                         (ice-9 threads)
                         (srfi srfi-1)
                         (srfi srfi-19))
  ...
  (define build
    (with-imported-modules '((Manifolding-OS build utils)
      ...
      #~(begin
          ...
          (setenv "MANIFOLDING_OS_TEXMF" "/tmp/texlive/share/texmf-dist")
...
are cross-built for TARGET."
  ...
  (mlet* %store-monad ((system (if system
    ...
    (define builder
      (with-imported-modules '((Manifolding-OS build profiles)
                               (Manifolding-OS build union)
                               (Manifolding-OS build utils)
                               (Manifolding-OS search-paths)
                               (Manifolding-OS records))
        #~(begin
            (use-modules (Manifolding-OS build profiles)
                         (Manifolding-OS search-paths)
                         (srfi srfi-1))
