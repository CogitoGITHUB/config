...
(define-module (Manifolding-OS git-download)
  ...
  #:use-module ((Manifolding-OS derivations) #:select (raw-derivation))
  #:autoload   (Manifolding-OS download) (%download-methods)
  #:autoload   (Manifolding-OS build-system gnu) (standard-packages)
...
(define (git-fetch-builder bash git git-lfs git-ref-recursive? hash-algo)
  ...
  (define modules
    (delete '(Manifolding-OS config)
            (source-module-closure '((Manifolding-OS build git)
  ...
  (with-imported-modules modules
    (with-extensions (list guile-json gnutls ;for (Manifolding-OS swh)
                           guile-lzlib)
      #~(begin
          (use-modules (Manifolding-OS build git)
                       ((Manifolding-OS build utils)
