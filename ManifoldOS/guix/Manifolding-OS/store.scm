...
(define %daemon-socket-uri
  ...
  (make-parameter (or (getenv "MANIFOLDING_OS_DAEMON_SOCKET")
                      %default-socket-path)))
            ...
            (match (uri-scheme uri)
              ...
              ((? symbol? scheme)
               ...
               (match (false-if-exception
                       (resolve-interface `(Manifolding-OS store ,scheme)))
                 ((? module? module)
                  (match (false-if-exception
...
(define current-store-protocol-version
  ...
  ;; purposes of (Manifolding-OS status) though.
  (make-parameter #f))
...
(define* (set-build-options server
                            ...
                            ;; traces" for use by (Manifolding-OS status).  Old clients
...
(define profiled?
  (let ((profiled
         (or (and=> (getenv "MANIFOLDING_OS_PROFILING") string-tokenize)
             '())))
    (lambda (component)
...
(define (file-mapping->tree mapping)
  ...
  ((\"Manifolding-OS/build/utils.scm\" . \"…/utils.scm\"))
...
could not be found."
  (let* ((base    (basename drv))
         (log     (string-append (or (getenv "MANIFOLDING_OS_LOG_DIRECTORY")
