...
(define-module (Manifolding-OS gexp)
...
false, meaning that GEXP is a plain Scheme object, return the empty list."
  ...
  (define (validate-modules gexp modules)
    ...
    ;; (Manifolding-OS config) as well as modules that come with Guile.
...
(define default-guile-derivation
  ...
  (let ((proc (delay
                (let ((iface (resolve-interface '(Manifolding-OS packages))))
...
EXP---e.g., '((Manifolding-OS build utils) (Manifolding-OS build gnu-build-system)).
...
to the source files instead of copying them."
  ...
  (mlet %store-monad ((files (mapm %store-monad file-pair files)))
    ...
    (gexp->derivation name build
                      ...
                      ;; constants in (Manifolding-OS build utils).
                      #:env-vars
...
by an arrow followed by a file-like object.  For example:
  ...
  (imported-modules `((Manifolding-OS build utils)
                      (Manifolding-OS gcrypt)
                      ((Manifolding-OS config) => ,(scheme-file …))))
      ...
      (gexp-with-hidden-inputs
       (gexp
        (begin
          ...
          ;; Above we loaded our own (Manifolding-OS build utils) but now we may need to
