...
(define-module (Manifolding-OS build minify-build-system)
  #:use-module ((Manifolding-OS build gnu-build-system) #:prefix gnu:)
  #:use-module ((Manifolding-OS build minify-build-system) #:prefix minify:)
...
(define* (build #:key javascript-files
  ...
  (let ((files (or javascript-files
                   (find-files "src" "\\.js$"))))
    (mkdir-p "Manifolding-OS/build")
    (for-each (cut minify <> #:directory "Manifolding-OS/build/") files)))
...
(define* (install #:key outputs #:allow-other-keys)
  (let* ((out (assoc-ref outputs "out"))
    ...
    (for-each
      ...
      (find-files "Manifolding-OS/build" "\\.min\\.js$"))))
