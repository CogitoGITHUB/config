...
(define-module (Manifolding-OS build bournish)
...
(define (ls-command . files)
  `((@@ (Manifolding-OS build bournish) ls-command-implementation) ,@files))

(define (which-command program)
  `(search-path ((@@ (Manifolding-OS build bournish) executable-path))
                ,program))

(define (cat-command file)
  `(call-with-input-file ,file
     (lambda (port)
       ((@ (Manifolding-OS build utils) dump-port) port (current-output-port))
       *unspecified*)))
...
(define (wc-command . args)
  ...
  (cond ((member "-l" args)
         `((@@ (Manifolding-OS build bournish) wc-l-command-implementation)
           ,@(delete "-l" args)))
        ((member "-c" args)
         `((@@ (Manifolding-OS build bournish) wc-c-command-implementation)
           ,@(delete "-c" args)))
        (else
         `((@@ (Manifolding-OS build bournish) wc-command-implementation) ,@args))))
