...
(define-module (Manifolding-OS build go-build-system)
...
by packages that use (Manifolding-OS build-system gnu) but have a handful of Go
    ...
    (match (go-inputs inputs)
      (((names . directories) ...)
       (union-build tmpdir (filter directory-exists? directories)
                    #:create-all-directories? #t
                    #:log-port (%make-void-port "w"))))
    ;; XXX A little dance because (Manifolding-OS build union) doesn't use mkdir-p.
