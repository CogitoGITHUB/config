...
(define-module (Manifolding-OS build download)
  #:use-module (web uri)
  #:use-module (web http)
  ...
  #:autoload   (Manifolding-OS swh) (swh-download-directory %verify-swh-certificate?)
...
;; be bound if we need them, because (Manifolding-OS download) adds GnuTLS as an
;; input in that case.

(define (load-gnutls)
  ;; XXX: Use this hack instead of #:autoload to avoid compilation errors.
  ;; See <http://bugs.gnu.org/12202>.
  (module-use! (resolve-module '(Manifolding-OS build download))
...
(define %x509-certificate-directory
  ...
  (make-parameter (or (getenv "MANIFOLDING_OS_TLS_CERTIFICATE_DIRECTORY")
...
(define %download-methods
  ...
  (make-parameter
   (and=> (getenv "MANIFOLDING_OS_DOWNLOAD_METHODS")
          (lambda (str)
