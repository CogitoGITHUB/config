...
(define-module (Manifolding-OS build pyproject-build-system)
...
(define (add-installed-pythonpath inputs outputs)
  "Prepend the site-package of OUTPUT to MANIFOLDING_OS_PYTHONPATH.  This is useful when
running checks after installing the package."
  (setenv "MANIFOLDING_OS_PYTHONPATH" (string-append (site-packages inputs outputs) ":"
                 ...
                 ((python-versioneer)
                  (let ((bundled "versioneer.py"))
                    (when (file-exists? bundled)
                      (delete-file bundled)))
                  (setenv "MANIFOLDING_OS_VERSIONEER_VERSION" version))
...
(define* (wrap #:key inputs outputs #:allow-other-keys)
  ...
  (let* ((var `("MANIFOLDING_OS_PYTHONPATH" prefix
                ,(search-path-as-string->list
                  (or (getenv "MANIFOLDING_OS_PYTHONPATH") "")))))
    (for-each (lambda (dir)
