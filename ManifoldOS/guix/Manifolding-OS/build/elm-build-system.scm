...
(define-module (Manifolding-OS build elm-build-system)
  #:use-module ((Manifolding-OS build gnu-build-system) #:prefix gnu:)
...
with the union of the Elm \"package\" inputs.  Also, set MANIFOLDING_OS_ELM_VERSION to
the version of the Elm compiler in use."
  (let* ((elm (search-input-file (or native-inputs inputs) "/bin/elm"))
    ...
    (setenv "MANIFOLDING_OS_ELM_VERSION" elm-version)
    (mkdir "../elm-home")
...
directory and link it into the ELM_HOME tree.  Also, set MANIFOLDING_OS_ELM_PKG_NAME and
MANIFOLDING_OS_ELM_PKG_VERSION to the name and version, respectively, of the Elm package
being built, as defined in its \"elm.json\" file."
  (let* ((elm-version (getenv "MANIFOLDING_OS_ELM_VERSION"))
    ...
    (setenv "MANIFOLDING_OS_ELM_PKG_NAME" name)
    (setenv "MANIFOLDING_OS_ELM_PKG_VERSION" version)
    (mkdir-p staged-dir)
...
MANIFOLDING_OS_ELM_OFFLINE_REGISTRY_FILE to its path, cooperating with a patch to `elm`
...
versions from the internet."
  (let* ((elm-home (getenv "ELM_HOME"))
         (elm-version (getenv "MANIFOLDING_OS_ELM_VERSION"))
         (registry-file
    ...
    (setenv "MANIFOLDING_OS_ELM_OFFLINE_REGISTRY_FILE" registry-file)))

(define (read-offline-registry->vhash)
  "Return a vhash mapping Elm \"package\" names to lists of available version
strings."
  (alist->vhash
   (call-with-input-file (getenv "MANIFOLDING_OS_ELM_OFFLINE_REGISTRY_FILE")
     json->scm)))
...
MANIFOLDING_OS_ELM_OFFLINE_REGISTRY_FILE environment variable is used to find available
versions."
...
\"package\" currently being built."
  (let* ((info (call-with-input-file "elm.json" json->scm))
         (name (getenv "MANIFOLDING_OS_ELM_PKG_NAME"))
         (version (getenv "MANIFOLDING_OS_ELM_PKG_VERSION"))
         (elm-home (getenv "ELM_HOME"))
...
installed."
  (let ((base (string-append "/share/elm/"
                             (getenv "MANIFOLDING_OS_ELM_VERSION")
                             "/packages/"
                             (getenv "MANIFOLDING_OS_ELM_PKG_NAME")
                             "/"
                             (getenv "MANIFOLDING_OS_ELM_PKG_VERSION")))
