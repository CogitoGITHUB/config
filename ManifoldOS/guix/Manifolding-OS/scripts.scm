...
(define-module (Manifolding-OS scripts)
  ...
  #:autoload   (Manifolding-OS describe) (current-profile-date)
  #:autoload   (Manifolding-OS build syscalls) (statfs
...
(define (environment-build-options)
  ...
  (arguments-from-environment-variable "MANIFOLDING_OS_BUILD_OPTIONS"))
...
is true, also pass arguments passed via the 'MANIFOLDING_OS_BUILD_OPTIONS' environment
variable.  Command-line options take precedence those passed via
'MANIFOLDING_OS_BUILD_OPTIONS'.
...
(define %distro-age-warning
  ...
  (make-parameter (match (and=> (getenv "MANIFOLDING_OS_DISTRO_AGE_WARNING")
                                string->duration)
                    (#f  (* 7 24 3600))
                    (age (time-second age)))))
...
(define* (warn-about-old-distro #:optional (old (%distro-age-warning))
  ...
  (when (and (or (not age) (>= age old))
             (not (getenv "MANIFOLDING_OS_UNINSTALLED")))
...
(define %disk-space-warning
  ...
  ;; MANIFOLDING_OS_DISK_SPACE_WARNING can contain both thresholds.  A value in [0;100)
  ...
  (let* ((default-absolute-threshold (size->number "5GiB"))
    ...
    (make-parameter
     (match (getenv "MANIFOLDING_OS_DISK_SPACE_WARNING")
