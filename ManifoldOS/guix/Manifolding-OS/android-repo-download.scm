...
(define-module (Manifolding-OS android-repo-download)
...
generic name if unset."
  ...
  (define config.scm
    (scheme-file "config.scm"
                 #~(begin
                     (define-module (Manifolding-OS config)
                       #:export (%libz))
  ...
  (define modules
    (cons `((Manifolding-OS config) => ,config.scm)
          (delete '(Manifolding-OS config)
                  (source-module-closure '((Manifolding-OS build android-repo)
                                           (Manifolding-OS build utils)
                                           (Manifolding-OS build download-nar))))))

  (define build
    (with-imported-modules modules
      (with-extensions (list gnutls guile-json ;for (Manifolding-OS swh)
                             guile-lzlib)
        #~(begin
            (use-modules (Manifolding-OS build android-repo)
                         (Manifolding-OS build utils)
                         (Manifolding-OS build download-nar)
                         (ice-9 match))
