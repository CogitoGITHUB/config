...
(define-module (Manifolding-OS gnupg)
  ...
  #:use-module ((Manifolding-OS utils) #:select (config-directory))
...
(define %gpg-command
  ...
  (make-parameter (or (getenv "MANIFOLDING_OS_GPG_COMMAND") "gpg")))

(define %gpgv-command
  ;; The 'gpgv' program.
  (make-parameter (or (getenv "MANIFOLDING_OS_GPGV_COMMAND") "gpgv")))
