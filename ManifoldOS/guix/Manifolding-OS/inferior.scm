...
(define-module (Manifolding-OS inferior)
  ...
  #:autoload   (Manifolding-OS build utils) (mkdir-p)
              ...
              (open-bidirectional-pipe
               ...
               (object->string
                `(begin
                   ...
                   ((@ (Manifolding-OS repl) machine-repl))))))))
        (values pipe pid))))
  ...
  (match (read pipe)
    (('repl-version 0 rest ...)
     (letrec ((result (inferior 'pipe pipe close (cons 0 rest)
       ...
       (inferior-eval '(use-modules (Manifolding-OS)) result)
...
(define* (read-repl-response port #:optional inferior)
  "Read a (Manifolding-OS repl) response from PORT and return it as a Scheme object.
...
TRANSLATE? is true, translate it to the current locale's language."
  (inferior-package-field package
                          (if translate?
                              '(compose (@ (Manifolding-OS ui) P_) package-synopsis)
                              'package-synopsis)))
...
TRANSLATE? is true, translate it to the current locale's language."
  (inferior-package-field package
                          (if translate?
                              '(compose (@ (Manifolding-OS ui) P_) package-description)
                              'package-description)))
  ...
  (define paths
    (inferior-package-field package
                            `(compose (lambda (paths)
                                        (map (@ (Manifolding-OS search-paths)
...
is similar to the sexp returned by 'package-provenance' for regular packages."
  (inferior-package-field package
                          '(let* ((describe
                                   (false-if-exception
                                    (resolve-interface '(Manifolding-OS describe))))
                                  (provenance
                                   (false-if-exception
                                    (module-ref describe
                                                'package-provenance))))
...
(define (proxy inferior store)                    ;adapted from (Manifolding-OS ssh)
