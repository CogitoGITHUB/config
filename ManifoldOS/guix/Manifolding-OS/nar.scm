...
(define-module (Manifolding-OS nar)
  #:use-module (Manifolding-OS serialization)
  #:autoload   (Manifolding-OS build syscalls) (lock-file unlock-file)
  #:use-module ((Manifolding-OS build utils)
                #:select (delete-file-recursively with-directory-excursion))

  ;; XXX: Eventually we should use (Manifolding-OS store database) exclusively, and not
  ;; (Manifolding-OS store) since this is "daemon-side" code.
  #:use-module (Manifolding-OS store)
  #:use-module (Manifolding-OS store database)
  #:use-module ((Manifolding-OS store deduplication) #:select (dump-file/deduplicate))
  #:use-module ((Manifolding-OS build store-copy) #:select (store-info))
