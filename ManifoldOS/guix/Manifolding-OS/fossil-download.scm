...
(define-module (Manifolding-OS fossil-download)
  #:use-module (Manifolding-OS build-system)
  #:use-module ((Manifolding-OS download) #:select (%disarchive-mirrors))
...
HASH-ALGO (a symbol).  Use NAME as the file name, or a generic name if #f."
  (let* ((modules (source-module-closure '((Manifolding-OS build fossil)
         ...
         (build
          (with-imported-modules modules
            (with-extensions extensions
              #~(begin
                  (use-modules (Manifolding-OS build fossil)
                               ((Manifolding-OS build download)
    ...
    (mlet %store-monad ((guile (package->derivation guile system)))
      (gexp->derivation tarball-name build
                        #:leaked-env-vars '("http_proxy" "https_proxy"
                                            "COLUMNS" "USER")
                        #:env-vars (match (getenv "MANIFOLDING_OS_DOWNLOAD_METHODS")
                                     (#f '())
                                     (value
