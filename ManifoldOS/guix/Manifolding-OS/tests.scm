...
(define-module (Manifolding-OS tests)
  #:use-module ((Manifolding-OS config) #:select (%storedir %localstatedir))
  ...
  #:use-module ((Manifolding-OS build utils) #:select (mkdir-p compressor))
...
(define %tests-build-timeout
  ...
  (let ((default (if (string=? "riscv64-linux" (%current-system))
                     ...
                     (* 10 60)
                     (* 5 60))))
    (match (getenv "MANIFOLDING_OS_TESTS_BUILD_TIMEOUT")
      (#f default)
...
(define %test-substitute-urls
  ...
  (make-parameter
   (or (and=> (getenv "MANIFOLDING_OS_BINARY_SUBSTITUTE_URL") list)
       '())))
    ...
    (let ((store-variable (getenv "NIX_STORE_DIR")))
      (dynamic-wind
        (lambda ()
          ...
          ;; influences '%store-directory' in (Manifolding-OS build utils), which is
          ;; itself used in (Manifolding-OS packages).  Thus, unset it before going any
          ;; further.
...
(define (random-seed)
  (or (and=> (getenv "MANIFOLDING_OS_TESTS_RANDOM_SEED")
             number->string)
...
(define %substitute-directory
  (make-parameter
   (and=> (getenv "MANIFOLDING_OS_BINARY_SUBSTITUTE_URL")
  ...
  (let* ((ext (string-index-right name #\.))
         ...
         (command #~(if #+comp
                        (string-append #+%bootstrap-coreutils&co
                                       "/bin/" #+comp)
                        #f))
         (f (with-imported-modules '((Manifolding-OS build utils))
              (computed-file name
                             #~(begin
