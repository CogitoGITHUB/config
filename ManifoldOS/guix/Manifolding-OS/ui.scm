...
(define-module (Manifolding-OS ui)                       ;import in user interfaces only
  ...
  #:autoload   (Manifolding-OS build-system) (build-system-name)
  #:use-module (Manifolding-OS serialization)
  #:autoload   (Manifolding-OS licenses) (license?
                                license-name
                                license-uri)
  #:autoload   (Manifolding-OS build syscalls) (free-disk-space
                                      terminal-columns
                                      terminal-rows
  ...
  #:re-export (G_ N_ P_                           ;now in (Manifolding-OS i18n)

               warning info report-error leave    ;now in (Manifolding-OS diagnostics)
               location->string
...
and defining @code{MANIFOLDING_OS_LOCPATH}, along these lines:
...
(define %guix-user-module
  ...
  ;; Compute lazily to avoid circularity with (Manifolding-OS gexp).
  (delay
    (let ((module (make-module)))
      (beautify-user-module! module)
      ;; Use (Manifolding-OS gexp) so that one can use #~ & co.
      (module-use! module (resolve-interface '(Manifolding-OS gexp)))
      module)))
...
available."
  (or (getenv "MANIFOLDING_OS_PAGER")
      (getenv "PAGER")
      (which "less")
      (which "more")
      ""))
...
(define* (command-files #:optional directory)
  ...
  (define directory*
    (or directory
        (and=> (search-path %load-path "Manifolding-OS.scm")
...
(define (extension-directories)
  ...
  (filter file-exists?
          (parse-path
           (getenv "MANIFOLDING_OS_EXTENSIONS_PATH"))))
...
Run COMMAND with ARGS, if given.\n"))
  ...
  (let ((commands   (commands))
        (categories (module-ref (resolve-interface '(Manifolding-OS scripts))
                                '%command-categories)))
    (for-each (match-lambda
                (('internal . _)
  ...
  (define module
    ...
    (match (search-path (extension-directories)
      ...
      (#f
       (catch 'misc-error
         (lambda ()
           (resolve-interface `(Manifolding-OS scripts ,command)))
         (lambda _
      ...
      (file
       ...
       (let ((maybe-extension-path
         ...
         (if (string-suffix? maybe-extension-path file)
             (resolve-interface `(Manifolding-OS extensions ,command))
             (resolve-interface `(Manifolding-OS scripts ,command)))))))
