...
(define-module (Manifolding-OS import utils)
  #:autoload   (git structs) (git-error-message)
  #:use-module (Manifolding-OS base32)
  #:use-module ((Manifolding-OS build download) #:prefix build:)
  ...
  #:use-module ((Manifolding-OS i18n) #:select (G_))
  #:use-module (Manifolding-OS store)
  #:use-module (Manifolding-OS download)
  #:use-module (Manifolding-OS sets)
  #:use-module ((Manifolding-OS ui) #:select (fill-paragraph))
...
Unless overwritten with REF, the ref (as defined by the (Manifolding-OS git) module) is
...
license object exported from (Manifolding-OS licenses).  Return #f if STR does not match
...
object is bound to in the (Manifolding-OS licenses) module, such as 'license:gpl3+, or
#f if there is no such known license."
  (define licenses
    (module-map (lambda (sym var) `(,(variable-ref var) . ,sym))
                (resolve-interface '(Manifolding-OS licenses) #:prefix 'license:)))
...
(define (build-system-modules)
  (modules-from-current-profile "Manifolding-OS/build-system"))
    ...
    (orig (let ((sha (match (assoc-ref orig "sha256")
            ...
            (origin
              (method (match (assoc-ref orig "method")
                        ("url-fetch" (@ (Manifolding-OS download) url-fetch))
                        ("git-fetch" (@ (Manifolding-OS git-download) git-fetch))
                        ("svn-fetch" (@ (Manifolding-OS svn-download) svn-fetch))
                        ("hg-fetch"  (@ (Manifolding-OS hg-download) hg-fetch))
                        (_ #f)))
     ...
     (match (assoc-ref meta "license")
       ...
       (l
        (or (false-if-exception
             (module-ref (resolve-interface '(Manifolding-OS licenses))
                         (string->symbol l)))
            (false-if-exception
             (module-ref (resolve-interface '(Manifolding-OS licenses) #:prefix 'license:)
