...
(define-module (Manifolding-OS import cran)
  ...
  #:use-module ((Manifolding-OS build-system r) #:select (cran-uri bioconductor-uri))
...
statements in files in the directory DIR."
  (if (getenv "MANIFOLDING_OS_CRAN_IGNORE_TEST_INPUTS")
      (set)
...
in vignette files in the directory DIR."
  (if (getenv "MANIFOLDING_OS_CRAN_IGNORE_VIGNETTE_INPUTS")
      (set)
      (match (scandir dir (negate (cute member <> '("." ".."))))
        ((package-directory-name . rest)
  ...
  (let ((input-names (map upstream-input-name inputs))
        (esbuild-modules '(#:modules
                           '((Manifolding-OS build r-build-system)
                             ((Manifolding-OS build minify-build-system) #:select (minify))
