...
(define-module (Manifolding-OS import github)
  ...
  #:use-module ((Manifolding-OS utils) #:select (string-replace-substring))
  #:use-module (Manifolding-OS i18n)
  #:use-module (Manifolding-OS diagnostics)
  #:use-module ((Manifolding-OS ui) #:select (display-hint))
  #:use-module ((Manifolding-OS download) #:prefix download:)
  #:use-module ((Manifolding-OS git-download) #:prefix download:)
  #:autoload   (Manifolding-OS build download) (open-connection-for-uri)
  #:use-module (json)
...
(define %github-token
  ...
  ;; limit, or #f.
  (make-parameter (getenv "MANIFOLDING_OS_GITHUB_TOKEN")))
...
setting the @env{MANIFOLDING_OS_GITHUB_TOKEN} environment variable to a token obtained
