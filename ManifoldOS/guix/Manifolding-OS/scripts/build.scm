...
(define-module (Manifolding-OS scripts build)
  ...
  #:autoload   (Manifolding-OS http-client) (http-fetch http-get-error?)
  #:autoload   (Manifolding-OS scripts graph) (%bag-node-type)
  #:autoload   (Manifolding-OS graph) (node-back-edges)
  #:autoload   (Manifolding-OS sets) (setq set-contains? set-insert)
  ...
  #:use-module ((Manifolding-OS progress) #:select (current-terminal-columns))
  #:autoload   (Manifolding-OS build syscalls) (terminal-columns)
...
;; XXX: The following procedure cannot be in (Manifolding-OS store) because of the
;; dependency on (Manifolding-OS derivations).
...
'%standard-build-options', set the corresponding build options on STORE."
  ...
  ;; case where MANIFOLDING_OS_DAEMON_SOCKET=guix://….
