...
(define-module (Manifolding-OS scripts locate)
  #:use-module ((Manifolding-OS config) #:select (%localstatedir))
  ...
  #:autoload   (Manifolding-OS combinators) (fold2)
  #:autoload   (Manifolding-OS grafts) (%graft?)
  #:autoload   (Manifolding-OS store roots) (gc-roots)
  ...
  #:use-module ((Manifolding-OS utils) #:select (cache-directory))
  #:autoload   (Manifolding-OS build utils) (find-files mkdir-p)
...
(define (last-insert-row-id db)        ;XXX: copied from (Manifolding-OS store database)
