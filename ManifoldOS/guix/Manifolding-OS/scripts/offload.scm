...
(define-module (Manifolding-OS scripts offload)
  ...
  #:autoload   (Manifolding-OS ssh) (authenticate-server*
                           connect-to-remote-daemon
                           send-files retrieve-files retrieve-files*
                           remote-inferior report-guile-error)
  #:use-module (Manifolding-OS store)
  #:autoload   (Manifolding-OS inferior) (inferior-eval close-inferior
                                inferior? inferior-protocol-error?)
  #:autoload   (Manifolding-OS derivations) (read-derivation-from-file
                                   derivation-file-name
                                   build-derivations)
  #:autoload   (Manifolding-OS nar) (restore-file-set)
  ...
  #:use-module ((Manifolding-OS build utils) #:select (which mkdir-p))
...
(define %user-module
  ...
  (let ((module (make-fresh-user-module)))
    (module-use! module (resolve-interface '(Manifolding-OS scripts offload)))
    module))
...
(define (assert-node-has-guix node name)
  "Bail out if NODE if #f or if we fail to use the (Manifolding-OS) module, or if its
daemon is not running."
  ...
  (match (inferior-eval '(begin
                        ...
                        node)
    ('alright #t)
    (_ (leave (G_ "(Manifolding-OS) module not usable on remote host '~a'")
              name)))
