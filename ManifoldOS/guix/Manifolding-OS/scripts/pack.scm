...
(define-module (Manifolding-OS scripts pack)
  #:use-module (Manifolding-OS scripts)
  #:autoload   (Manifolding-OS import json) (json->scheme-file)
  ...
  #:autoload   (Manifolding-OS inferior) (inferior-package?
  ...
  #:use-module ((Manifolding-OS self) #:select (make-config.scm))
  #:use-module ((Manifolding-OS docker) #:select (%docker-image-max-layers))
      ...
      #~(begin
          (setenv "MANIFOLDING_OS_LOCPATH"
                  #+(file-append (let-system (system target)
                                 ...
                                 "/lib/locale"))
          (setlocale LC_ALL "en_US.utf8"))
      #~(setenv "MANIFOLDING_OS_LOCPATH" "unset for tests")))
  ...
  (gexp->derivation
   ...
   (with-extensions (if deduplicate? (list guile-gcrypt) '())
     (with-imported-modules (let ((lst (source-module-closure
                              ...
                              (if deduplicate?
                                  lst
                                  (delete '(Manifolding-OS store deduplication) lst)))

       (source-module-closure '((Manifolding-OS build pack)
                                (Manifolding-OS build utils))
                              #:select? not-config?)
       #~(begin
      ...
      (with-imported-modules (source-module-closure
        ...
        #~(begin
            (use-modules (Manifolding-OS build utils)
                         (Manifolding-OS build store-copy)
                         ((Manifolding-OS build union) #:select (relative-file-name))
                         (gnu build install)
                         (srfi srfi-1)
                         (srfi srfi-26)
                         (ice-9 match))
...
layers."
  ...
  (define build
    ;; Guile-JSON and Guile-Gcrypt are required by (Manifolding-OS docker).
    (with-extensions (list guile-json-3 guile-gcrypt)
      (with-imported-modules `(((Manifolding-OS config) => ,(make-config.scm))
                               ,@(source-module-closure
                                  `((Manifolding-OS docker)
                                    (Manifolding-OS build store-copy)
                                    (Manifolding-OS build utils) ;for %xz-parallel-args
                                    (Manifolding-OS profiles)
                                    (Manifolding-OS search-paths))
                                  #:select? not-config?))
        #~(begin
            (use-modules (Manifolding-OS docker) (Manifolding-OS build store-copy)
                         (Manifolding-OS build utils)
                         (Manifolding-OS profiles) (Manifolding-OS search-paths)
  ...
  (define run
    (with-imported-modules (source-module-closure
                            '((Manifolding-OS build utils)
                              (Manifolding-OS search-paths)))
      #~(begin
          (use-modules (Manifolding-OS build utils) (Manifolding-OS search-paths)
                       (ice-9 match))
...
libfakechroot.so and related ld.so machinery as a fallback."
  ...
  (define build
    (with-imported-modules (source-module-closure
      ...
      #~(begin
          (use-modules (Manifolding-OS build utils)
                       ((Manifolding-OS build union) #:select (symlink-relative))
                       (Manifolding-OS elf)
                       (Manifolding-OS build gremlin)
                       (ice-9 binary-ports)
                       (ice-9 ftw)
         ...
         (cond
          ...
          ((not (null? manifests))
           (concatenate-manifests
            (map (lambda (file)
                   (load* file '((Manifolding-OS profiles) (gnu))))
                 manifests)))
          (else
