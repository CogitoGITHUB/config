...
(define-module (Manifolding-OS scripts home)
  ...
  #:autoload   (Manifolding-OS colors) (supports-hyperlinks? file-hyperlink)
  #:use-module (Manifolding-OS packages)
  #:use-module (Manifolding-OS profiles)
  #:use-module (Manifolding-OS store)
  #:use-module (Manifolding-OS utils)
  #:autoload   (Manifolding-OS graph) (lookup-backend export-graph)
  #:use-module (Manifolding-OS scripts)
  #:use-module (Manifolding-OS scripts package)
  #:use-module (Manifolding-OS scripts build)
  #:autoload   (Manifolding-OS scripts system search) (service-type->recutils)
  #:use-module (Manifolding-OS scripts system reconfigure)
  #:autoload   (Manifolding-OS scripts system) (service-node-type
                                      shepherd-service-node-type)
                                      shepherd-service-node-type)
  #:autoload   (Manifolding-OS scripts home edit) (guix-home-edit)
  #:autoload   (Manifolding-OS scripts home import) (import-manifest)
  #:use-module ((Manifolding-OS status) #:select (with-status-verbosity))
  #:use-module ((Manifolding-OS build utils) #:select (mkdir-p switch-symlinks))
     ...
     (with-imported-modules `(((Manifolding-OS config) => ,(make-config.scm))
       ...
       #~(begin
           (use-modules (Manifolding-OS build utils)
                        ((Manifolding-OS profiles) #:select (load-profile)))
           ...
           (setenv "MANIFOLDING_OS_NEW_HOME" #$home)
           (primitive-load (string-append #$home "/activate"))
           (setenv "MANIFOLDING_OS_NEW_HOME" #f)
           ...
           (case action
             ((reconfigure)
              (let* ((number (generation-number %guix-home))
                ...
                (setenv "MANIFOLDING_OS_NEW_HOME" he-out-path)
...
;; TODO: Make it public in (Manifolding-OS scripts system)
...
SPEC.  STORE is an open connection to the store."
  (let* ((number (relative-generation-spec->number %guix-home spec))
    ...
    (if number
        (begin
          (setenv "MANIFOLDING_OS_NEW_HOME" (readlink generation))
