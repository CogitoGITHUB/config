...
(define-module (Manifolding-OS scripts environment)
  #:use-module (Manifolding-OS ui)
  #:use-module (Manifolding-OS store)
  #:use-module (Manifolding-OS utils)
  #:use-module ((Manifolding-OS status) #:select (with-status-verbosity))
  ...
  #:autoload   (Manifolding-OS describe) (current-profile)
  #:use-module (Manifolding-OS scripts)
  #:use-module (Manifolding-OS scripts build)
  #:autoload   (Manifolding-OS scripts pack) (symlink-spec-option-parser)
  ...
  #:autoload   (Manifolding-OS build syscalls) (set-network-interface-up openpty login-tty)
  ...
  #:autoload   (gnu packages package-management) (Manifolding-OS)
...
use '--preserve' instead~%"))
         ...
         (option '(#\S "symlink") #t #f
                 (lambda (opt name arg result)
                   ;; Delay call to avoid auto-loading (Manifolding-OS scripts pack)
                   ;; when unnecessary.
...
(define (load-manifest file)                      ;TODO: factorize
  ...
  (let ((modules '((Manifolding-OS profiles) (gnu))))
    (load* file modules)))
...
and generate the LD cache."
  ...
  (setenv "MANIFOLDING_OS_ENVIRONMENT" profile)
...
suffix '~a'~%")
    ...
    ;; Special case.
    (match (vhash-assoc "MANIFOLDING_OS_ENVIRONMENT" actual)
      (#f
       (warn (G_ "'MANIFOLDING_OS_ENVIRONMENT' is missing from the shell \
environment~%")))
      ((_ . value)
       (unless (string=? value profile)
         (warn (G_ "'MANIFOLDING_OS_ENVIRONMENT' is set to '~a' instead of '~a'~%")
               value profile))))
      ...
      (match (vhash-assoc "PS1" actual)
        ...
        ((_ . str)
         (when (and (getenv "PS1") (string=? str (getenv "PS1"))
                    ...
                    ;; 'PS1' might be conditional on 'MANIFOLDING_OS_ENVIRONMENT', as
                    ;; shown in the hint below.
                    (not (or (string-contains str "$MANIFOLDING_OS_ENVIRONMENT")
                             (string-contains str "${MANIFOLDING_OS_ENVIRONMENT"))))
       ...
       (exit/status
        (call-with-container file-systems
          ...
          (lambda ()
            ...
            ;; If requested, link $MANIFOLDING_OS_ENVIRONMENT to $HOME/.guix-profile;
