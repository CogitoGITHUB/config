...
(define-module (Manifolding-OS svn-download)
  ...
  #:use-module ((Manifolding-OS build svn) #:prefix build:)
  #:use-module ((Manifolding-OS build utils) #:select (mkdir-p))
...
(define (svn-fetch-builder svn hash-algo)
  ...
  (define tar+gzip                                ;for (Manifolding-OS swh)
  ...
  (with-imported-modules
      (source-module-closure '((Manifolding-OS build svn)
                               (Manifolding-OS build download)
                               (Manifolding-OS build download-nar)
                               (Manifolding-OS build utils)
                               (Manifolding-OS swh)))
    (with-extensions (list guile-json guile-gnutls   ;for (Manifolding-OS swh)
                           guile-lzlib)
      #~(begin
          (use-modules (Manifolding-OS build svn)
                       ((Manifolding-OS build download)
    ...
    (gexp->derivation (or name "svn-checkout")
                      ...
                      `(("svn url" . ,(svn-reference-url ref))
                        ...
                        ,@(if (svn-reference-password ref)
                              ...
                              '())
                        ,@(match (getenv "MANIFOLDING_OS_DOWNLOAD_METHODS")
                            (#f '())
                            (value
                             `(("MANIFOLDING_OS_DOWNLOAD_METHODS" . ,value))))
                        ;; To avoid pulling in (Manifolding-OS base32) in the builder
