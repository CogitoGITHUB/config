...
(define-module (Manifolding-OS self)
  ...
  #:use-module (Manifolding-OS modules)
  #:use-module ((Manifolding-OS utils) #:select (version-major+minor))
  #:use-module ((Manifolding-OS build utils) #:select (find-files))
...
(define (translate-texi-manuals source)
  ...
  (define build
    (with-imported-modules '((Manifolding-OS build utils) (Manifolding-OS build po))
      #~(begin
          (use-modules (Manifolding-OS build utils) (Manifolding-OS build po)
...
instead of 'C'."
  ...
  (define build
    ;; XXX: Reuse <c-compiler> from (Manifolding-OS scripts pack) instead?
    (with-imported-modules '((Manifolding-OS build utils))
      #~(begin
          (use-modules (Manifolding-OS build utils)
                       (srfi srfi-26))
          ...
          (setenv "MANIFOLDING_OS_LD_WRAPPER_DISABLE_RPATH" "1")
...
load path."
  ...
  (program-file "guix-command"
                #~(begin
                    ...
                    (let ((locpath (getenv "MANIFOLDING_OS_LOCPATH")))
                      (setenv "MANIFOLDING_OS_LOCPATH"
                    ...
                    (let ((guix-main (module-ref (resolve-interface '(Manifolding-OS ui))
                                                 'guix-main)))
                      #$(if source
                            #~(begin
                                (bindtextdomain "guix"
    ...
    (with-imported-modules '((Manifolding-OS build utils))
      #~(begin
          ...
          (substitute* #$name
            ...
            (("@storedir@") #$%storedir)
            (("@prefix@") #$daemon)
            (("@MANIFOLDING_OS_SUBSTITUTE_URLS@")
  ...
  (define (wrap daemon)
    (program-file "guix-daemon"
                  #~(begin
                      ...
                      (unless (getenv "MANIFOLDING_OS_STATE_DIRECTORY")
                        (setenv "MANIFOLDING_OS_STATE_DIRECTORY"
                                #$(string-append %localstatedir "/guix")))
                      (unless (getenv "MANIFOLDING_OS_CONFIGURATION_DIRECTORY")
...
(define* (compiled-guix source #:key
                        ...
                        (Manifolding-OS (specification->package "guix")))
  ...
  (define *core-modules*
    (scheme-node "guix-core"
                 ...
                 ;; (Manifolding-OS man-db) is needed at build-time by (Manifolding-OS profiles)
                 ;; but we don't need to compile it; not compiling it allows
                 ;; us to avoid an extra dependency on guile-gdbm-ffi.
                 #:extra-files
                 `(("Manifolding-OS/man-db.scm" ,(local-file "../Manifolding-OS/man-db.scm"))
                   ("Manifolding-OS/build/po.scm" ,(local-file "../Manifolding-OS/build/po.scm"))
                   ("Manifolding-OS/store/schema.sql"
                    ,(local-file "../Manifolding-OS/store/schema.sql")))

                 #:extensions (list guile-gcrypt
                                    guile-git     ;for (Manifolding-OS git)
                                    guile-json)   ;for (Manifolding-OS swh)
                 #:guile-for-build guile-for-build))
  ...
  (define *extra-modules*
    (scheme-node "guix-extra"
                 ...
                 #:extra-files
                 `(("Manifolding-OS/graph.js" ,(local-file "../Manifolding-OS/graph.js"))
                   ("Manifolding-OS/d3.v3.js" ,(local-file "../Manifolding-OS/d3.v3.js")))
  ...
  (define *system-modules*
    (scheme-node "guix-system"
                 `((gnu system)
                   ...
                   ,@(scheme-modules* source "Manifolding-OS/platforms/"))
  ...
  (define *core-cli-modules*
    ...
    (scheme-node "guix-cli-core"
                 (remove (match-lambda
                           ...
                           (('guix 'pack) #t)
                           (_ #f))
                         (scheme-modules* source "Manifolding-OS/scripts"))
                 (list *core-modules* *extra-modules*
  ...
  (define *system-test-modules*
    ...
    (scheme-node "guix-system-tests"
                 ...
                 (list *core-package-modules* *package-modules*
                       ...
                       *cli-modules*)           ;for (Manifolding-OS scripts pack), etc.
  ...
  (define (built-modules node-subset)
    (directory-union (string-append name "-modules")
                     (append-map node-subset
                                 ...
                                 ;; overrides the (Manifolding-OS config) module that
...

;;;
;;; Generating (Manifolding-OS config).
;;;

(define %persona-variables
  ;; (Manifolding-OS config) variables that define Guix's persona.
  '(%guix-package-name
    %guix-version
    %guix-bug-report-address
    %guix-home-page-url))
...
(define %config-variables
  ;; (Manifolding-OS config) variables corresponding to Guix configuration.
  ...
  (scheme-file "config.scm"
               #~(;; The following expressions get spliced.
                   ...
                   (define %state-directory
                     ...
                     ;; `nix/local.mk'.
                     (or (getenv "MANIFOLDING_OS_STATE_DIRECTORY")
                         (string-append %localstatedir "/guix")))

                   (define %store-database-directory
                     (or (getenv "MANIFOLDING_OS_DATABASE_DIRECTORY")
                         (string-append %state-directory "/db")))

                   (define %config-directory
                     ;; This must match `MANIFOLDING_OS_CONFIGURATION_DIRECTORY' as
                     ;; defined in `nix/local.mk'.
                     (or (getenv "MANIFOLDING_OS_CONFIGURATION_DIRECTORY")
                         (string-append %sysconfdir "/guix")))
...
like '(\"Manifolding-OS/foo.scm\" \"gnu/bar.scm\") and MODULE-TREE is the directory
...
this PULL-VERSION value is not supported."
  ...
  (mbegin %store-monad
    ...
    (let ((Manifolding-OS (compiled-guix source
                               #:version version
