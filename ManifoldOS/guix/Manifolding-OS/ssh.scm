...
(define-module (Manifolding-OS ssh)
    ...
    `(begin
       ...
       (define connect-to-daemon
         ...
         (let ((connect-to-daemon
                (false-if-exception (module-ref (resolve-module '(Manifolding-OS store))
                                                'connect-to-daemon))))
           (lambda (uri)
       ...
       ;; Use 'connect-to-daemon' to honor MANIFOLDING_OS_DAEMON_SOCKET.
       (let ((sock    (connect-to-daemon (or (getenv "MANIFOLDING_OS_DAEMON_SOCKET")
                                             ,socket-name)))
...
can be written."
  ...
  (define import
    `(begin
       (use-modules (Manifolding-OS) (srfi srfi-34)
...
be read.  When RECURSIVE? is true, the closure of FILES is exported."
  ...
  (define export
    `(begin
       (use-modules (Manifolding-OS) (srfi srfi-1)
...
the machine on the other end of SESSION."
  (inferior-remote-eval '(begin (use-modules (Manifolding-OS utils)) (%current-system))
                        session))
