...
(define-module (Manifolding-OS search-paths)
  ...
  #:export (<search-path-specification>
            ...
            $LIBRARY_PATH
            $MANIFOLDING_OS_EXTENSIONS_PATH
            $PATH
            $PKG_CONFIG_PATH
            $SSL_CERT_DIR
            $SSL_CERT_FILE
...
(define $MANIFOLDING_OS_EXTENSIONS_PATH
  ;; 'MANIFOLDING_OS_EXTENSIONS_PATH' is used by Guix to locate extension commands.
...
;; Two variables for certificates (info "(Manifolding-OS)X.509 Certificates"),
...
corresponds to the arguments expected by `set-path-environment-variable'."
  ...
  ;; (Manifolding-OS profiles), so think twice before you change it.
  (match spec
...
;; XXX: This procedure used to be in (Manifolding-OS utils) but since we want to be able
;; to use (Manifolding-OS search-paths) on the build side, we want to avoid the
;; dependency on (Manifolding-OS utils), and so this procedure is back here for now.
