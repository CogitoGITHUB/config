...
export def ManifoldOS-Weather [
    ...
    if $data == null {
        print ""
        print $"(ansi white_bold)🌹 MANIFOLD // WEATHER 🌹(ansi reset)"
        print $"(ansi white)  Could not reach wttr.in — check network.(ansi reset)"
        print ""
        return
    }
    ...
    print ""
    print $"(ansi white_bold)🌹 MANIFOLD // WEATHER 🌹(ansi reset)"
    print $"(ansi white)  ($loc)(ansi reset)"
    print ""
    print $"(ansi white_bold)  NOW(ansi reset)"
    print $"(ansi white)  condition(ansi reset)   ($emoji) ($condition)"
    print $"(ansi white)  temperature(ansi reset) ($temp)  feels like ($feels)"
    print $"(ansi white)  humidity(ansi reset)    ($humidity)%"
    print $"(ansi white)  wind(ansi reset)        ($wind_kmph) km/h ($wind_dir)"
    print $"(ansi white)  visibility(ansi reset)  ($visibility) km"
    print $"(ansi white)  uv index(ansi reset)    ($uv)"
    print ""
    print $"(ansi white_bold)  FORECAST(ansi reset)"
    print $"(ansi white)  ($day0)(ansi reset)"
    print $"(ansi white)  ($day1)(ansi reset)"
    print $"(ansi white)  ($day2)(ansi reset)"
    print ""
}