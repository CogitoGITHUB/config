<h1 align=center>manifolding-shell</h1>
...
-   Dots: [`manifolding`](https://github.com/caelestia-dots)

## Installation

> [!NOTE]
> This repo is for the desktop shell of the manifolding dots. If you want installation instructions
> for the entire dots, head to [the main repo](https://github.com/caelestia-dots/manifolding) instead.
...
The shell is available from the AUR as `manifolding-shell`. You can install it with an AUR helper
like [`yay`](https://github.com/Jguer/yay) or manually downloading the PKGBUILD and running `makepkg -si`.

A package following the latest commit also exists as `manifolding-shell-git`. This is bleeding edge
...
The package is available as `manifolding-shell.packages.<system>.default`, which can be added to your
`environment.systemPackages`, `users.users.<username>.packages`, `home.packages` if using home-manager,
or a devshell. The shell can then be run via `manifolding-shell`.
...
For home-manager, you can also use the Manifolding's home manager module (explained in [configuring](https://github.com/caelestia-dots/shell?tab=readme-ov-file#home-manager-module)) that installs and configures the shell and the CLI.

### Manual installation

Dependencies:

-   [`manifolding-cli`](https://github.com/caelestia-dots/cli)
...
To install the shell manually, install all dependencies and clone this repo to `$XDG_CONFIG_HOME/quickshell/manifolding`.
Then simply build and install using `cmake`.

```sh
cd $XDG_CONFIG_HOME/quickshell
git clone https://github.com/caelestia-dots/shell.git manifolding
...
>
> e.g. installing to `~/.config/quickshell/manifolding` for easy local changes:
>
> ```sh
> mkdir -p ~/.config/quickshell/manifolding
> cmake -B build -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/ -DINSTALL_QSCONFDIR=~/.config/quickshell/manifolding
> cmake --build build
> sudo cmake --install build
> sudo chown -R $USER ~/.config/quickshell/manifolding
> ```

## Usage

The shell can be started via the `manifolding shell -d` command or `qs -c manifolding`.
If the entire manifolding dots are installed, the shell will be autostarted on login
via an `exec-once` in the hyprland config.

### Shortcuts/IPC

All keybinds are accessible via Hyprland [global shortcuts](https://wiki.hyprland.org/Configuring/Binds/#dbus-global-shortcuts).
If using the entire manifolding dots, the keybinds are already configured for you.
Otherwise, [this file](https://github.com/caelestia-dots/manifolding/blob/main/hypr/hyprland/keybinds.conf#L1-L39)
contains an example on how to use global shortcuts.

All IPC commands can be accessed via `manifolding shell ...`. For example

```sh
manifolding shell mpris getActive trackTitle
```

The list of IPC commands can be shown via `manifolding shell -s`:
...
by default. To change it, change the wallpapers path in `~/.config/manifolding/shell.json`.

To set the wallpaper, you can use the command `manifolding wallpaper`. Use `manifolding wallpaper -h` for more info about
the command.
...
If installed manually, you can update by running `git pull` in `$XDG_CONFIG_HOME/quickshell/manifolding`.

```sh
cd $XDG_CONFIG_HOME/quickshell/manifolding
git pull
```

## Configuring

All configuration options should be put in `~/.config/manifolding/shell.json`. This file is _not_ created by
default, you must create it manually. Options that you omit from the config file will use their default
values.

### Per-monitor configuration

You can configure options per-monitor in `~/.config/manifolding/monitors/<screen-name>/shell.json`. Options
...
**`~/.config/manifolding/monitors/DP-1/shell.json`**
    ...
    "launcher": {
        ...
        "actions": [
            ...
            },
            {
                ...
                "command": ["manifolding", "wallpaper", "-r"],
                "enabled": true,
                "dangerous": false
            },
            {
            ...
            },
            {
                ...
                "command": ["manifolding", "shell", "nexus", "open"],
                "enabled": true,
                "dangerous": false
            }
        ]
...
A separate `~/.config/manifolding/shell-tokens.json` file allows editing the internal tokens without
...
`~/.config/manifolding/monitors/<screen-name>/shell-tokens.json`.
...
programs.manifolding = {
  ...
  cli = {
    enable = true; # Also add manifolding-cli to path
    settings = {
...
The module automatically adds Manifolding shell to the path with **full functionality**. The CLI is not required, however you have the option to enable and configure it.
...
Try disabling VRR in the hyprland config. You can do this by adding the following to `~/.config/manifolding/hypr-user.conf`:
...
You can add your custom hyprland configs to `~/.config/manifolding/hypr-user.conf`.
...
Set a wallpaper via the launcher or `manifolding wallpaper` and set the scheme to the dynamic scheme via the launcher
or `manifolding scheme set`. e.g.

```sh
manifolding wallpaper -f <path/to/file>
manifolding scheme set -n dynamic
```
