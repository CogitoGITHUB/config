...
PageBase {
    ...
    ColumnLayout {
        ...
        // Parsed from the manifolding CLI's package listing; the sh wrapper avoids a
        // warning when the (optional) CLI isn't installed
        Process {
            running: true
            command: ["sh", "-c", "manifolding --version 2>/dev/null"]
            stdout: StdioCollector {
                onStreamFinished: {
                    const m = text.match(/manifolding-cli\S*\s+(\d+(?:\.\d+)*)/);
