...
Q_LOGGING_CATEGORY(lcSessionManager, "manifolding.services.sessionmanager", QtInfoMsg)
...
void SessionManager::hibernate() {
    if (queryHibernateAvailable()) {
        ...
        if (!engine)
            return;
        auto* const toaster = engine->singletonInstance<Toaster*>("Manifolding", "Toaster");
        if (!toaster)
            return;
        toaster->toast(
