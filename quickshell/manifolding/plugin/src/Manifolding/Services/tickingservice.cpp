...
TickingService::TickingService(QObject* parent)
    ...
    auto* dash = manifolding::config::GlobalConfig::instance()->dashboard();
    applyInterval(dash->resourceUpdateInterval());
    QObject::connect(dash, &manifolding::config::DashboardConfig::resourceUpdateIntervalChanged, this, [this, dash] {
        applyInterval(dash->resourceUpdateInterval());
    });
}
