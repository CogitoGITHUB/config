...
#define CONFIG_LIST_TYPE(Element, Name)                                                                                \
    class Name : public manifolding::config::ConfigList {                                                                \
        Q_OBJECT                                                                                                       \
        QML_ANONYMOUS                                                                                                  \
                                                                                                                       \
    public:                                                                                                            \
        explicit Name(QObject* parent = nullptr, const QVariantList& defaults = {})                                    \
            : manifolding::config::ConfigList(parent, defaults) {                                                        \
            resetToDefaults();                                                                                         \
        }                                                                                                              \
                                                                                                                       \
        Q_INVOKABLE manifolding::config::Element* at(int index) const {                                                  \
            return static_cast<manifolding::config::Element*>(itemAt(index));                                            \
        }                                                                                                              \
        Q_INVOKABLE manifolding::config::Element* insert(const QVariantMap& props, int index = -1) {                     \
            return static_cast<manifolding::config::Element*>(insertItem(props, index));                                 \
        }                                                                                                              \
                                                                                                                       \
    protected:                                                                                                         \
        [[nodiscard]] manifolding::config::ConfigObject* createItem() override {                                         \
            return new manifolding::config::Element(this);                                                               \
        }                                                                                                              \
    };
...
#define CONFIG_LIST(Type, name, ...)                                                                                   \
    Q_PROPERTY(manifolding::config::Type* name READ name CONSTANT)                                                       \
                                                                                                                       \
public:                                                                                                                \
...
public:                                                                                                                \
    [[nodiscard]] Type* name() const {                                                                                 \
        if (isOverlay())                                                                                               \
            qCWarning(manifolding::config::lcConfig, "Reading global-only option '%s' on per-monitor overlay",           \
